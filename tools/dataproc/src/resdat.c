...
#include "generated/narc.h" // IWYU pragma: keep
...
void parse_opts(int argc, const char **argv, options_t *opts) {
    for (int i = 0; i < argc - 1; i++) {
        if (strcmp(argv[i], "-p") == 0 || strcmp(argv[i], "--prefix") == 0) {
            ...
            i++;
        } else if (strcmp(argv[i], "-G") == 0 || strcmp(argv[i], "--gen-header") == 0) {
            opts->genheader = true;
...
void usage(void) {
    ...
    printf("  -G, --gen-header         If set, generate a C header for template-members\n");
}
...
static inline void generate_header(const char *path, const char *prefix, template_list_t *list) {
    char *full_path = prefix_path(path, prefix);
    ...
    const char *p_dirname   = dirname(full_path);
    const char *p_parentdir = strrchr(p_dirname, '/');
    const char *p_basename  = path;

    p_parentdir = p_parentdir ? p_parentdir + 1 : &p_dirname[0];

    size_t guard_size = snprintf(NULL, 0, "RESDAT_%s_%s\n", p_parentdir, p_basename);
    char  *guard      = malloc(guard_size);
    snprintf(guard, guard_size, "RESDAT_%s_%s\n", p_parentdir, p_basename);
    for (size_t i = 0; i < guard_size; i++) {
        if      (guard[i] >= 'a' && guard[i] <= 'z') guard[i] -= ('a' - 'A');
        else if (guard[i] == '.' || guard[i] == '-') guard[i] = '_';
    }

    fprintf(f, "/*\n");
    fprintf(f, " * THIS FILE WAS AUTOMATICALLY GENERATED BY resdat\n");
...
int main(int argc, const char **argv) {
    ...
    if (dataproc_anyerror(data)) goto cleanup_all;
    ...
    if (opts.genheader) generate_header("templates.h", opts.prefix, &templates);
...
cleanup_all:
    ...
    for (size_t i = 0; i < templates.size; i++) free((char *)templates.names[i]);
    free(templates.names);
...
static inline const char* get_member_string(dataproc_t *data, void *object, const char *key, const char *parent) {
    ...
    if (value == NULL) {
        raise_from(object, "expected '%s' to have member with key '%s'", parent, key);
        return NULL;
    }
...
template_list_t parse_templates(
    ...
    templates.names = calloc(templates_count,  sizeof(*templates.names));
    ...
    for (size_t i = 0; i < templates_count; i++) {
        ...
        templates.names[i] = malloc(strlen(id) + 1);
        strcpy((char *)templates.names[i], id);
