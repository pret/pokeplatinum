...
def parse_ev_flags(ev_flags) -> bytes:
    value = 0
    for i, flag in enumerate(("hp", "attack", "defense", "speed", "special_attack", "special_defense")):
        if ev_flags[flag]:
            value += 1 << i
    return u8(value)
...
packables.extend(parse_ev_flags(data['evFlags']))
