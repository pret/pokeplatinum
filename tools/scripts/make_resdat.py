...
RESDAT_TYPES = ["char", "pltt", "cell", "anim", "multicell", "multianim"]
...
type_idx = RESDAT_TYPES.index(data['kind'])
# for palette resdats, output a header which C sources can consume
output_header = data['kind'] == "pltt"
...
#### palette count field is only used for `pltt` resdats, other resdats fill this with the narc index
#### vram type field is only used for `pltt` and `char` resdats, other resdats fill this with the narc index
#### terminated with 6 ints of -2
with open(output_file, mode="wb") as f:
    f.write(struct.pack('<i', type_idx))
    for element in data['graphics']:
        f.write(struct.pack('<i', element['narc']))
        f.write(struct.pack('<i', element['file']))
...
header_file = output_file.with_suffix(".naix")
if output_header:
    ...
    with open(header_file, mode="w") as f:
        ...
        f.write(f"#ifndef RESDAT_{fn}_NAIX_H_\n")
        f.write(f"#define RESDAT_{fn}_NAIX_H_\n\n")
        f.write("enum {\n")
        running_offset = 0
        index = 0
        for element in data['graphics']:
            element_name = f"RESDAT_{fn}_NARC_{element['narc']}_MEMBER_{element['file']}_{index}_OFFSET"
