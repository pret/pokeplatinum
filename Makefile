.PHONY: configure rom check target format clean
...
rom:
	NINJA_STATUS="[%p %f/%t] " $(MESON) compile -C $(POKEPLATINUM_BUILD) "pokeplatinum.us.nds"

check:
	NINJA_STATUS="[%p %f/%t] " $(MESON) test -C $(POKEPLATINUM_BUILD)

target:
	NINJA_STATUS="[%p %f/%t] " $(MESON) compile -C $(POKEPLATINUM_BUILD) $(MESON_TARGET)
...
ifeq (0,$(WRAP_CONFIG))
	MWCONFIG=$(realpath -- $(DOT_MWCONFIG)) $(MWRAP) -conf -wine "$(WRAP_WINE)" -path_unx "$(WRAP_PATH_UNX)" -path_win "$(WRAP_PATH_WIN)" -path_build_unx "$(WRAP_PATH_BUILD_UNX)" -path_build_win "$(WRAP_PATH_BUILD_WIN)"
endif
	MWCONFIG=$(realpath -- $(DOT_MWCONFIG)) $(MESON) setup --wrap-mode=nopromote --native-file=meson/$(NATIVE) --native-file=$(ROOT_INI) --cross-file=meson/$(CROSS) --cross-file=$(ROOT_INI) -- $(POKEPLATINUM_BUILD)
