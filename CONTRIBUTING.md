...
We provide a script to generate a `compile_commands.json` compilation database.
This configuration file is compatible with most editors that support Language
Server plugins, such as Visual Studio Code, Vim, or Emacs.

To generate the compilation database, run:

```bash
./tools/devtools/gen_compile_commands.py
```

The `compile_commands.json` file will appear in the root of your repository.
Language servers such as `clangd` should detect its presence automatically on
startup. Visual Studio Code users will need to take extra steps to configure
their environment:

1. Open the `C/C++: Edit Configurations (UI)` view.
2. Scroll down to `Advanced Settings` and expand the section.
3. Scroll down to `Compile commands` and enter the following in the textbox:
...
For instructions on installing `pre-commit`, refer to the documentation [on
their website][pre-commit-install]. Then, in the root of your repository, run:
