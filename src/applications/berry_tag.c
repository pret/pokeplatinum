...
enum {
    BERRY_TAG_FADE_STATE_FADING_OUT,
...
static void BerryTag_LoadGraphics(BerryTag *berryTag, NARC *narc)
{
    ...
    Font_LoadTextPalette(0, 15 * 0x20, HEAP_ID_BERRY_TAG);
}
...
static void BerryTag_LoadBerryGraphics(BerryTag *berryTag, NARC *narc)
{
    ...
    Graphics_LoadPaletteFromOpenNARC(narc, BerryTag_GetPalette(berryTag->outsideData->currentBerryID), PAL_LOAD_MAIN_BG, 3 * 0x20, 0x20, HEAP_ID_BERRY_TAG);
}
    ...
    if (direction < 0) {
        if (berryID != 0) {
            while (TRUE) {
                ...
                if (BerryTagData_Contains(entries, v0) == TRUE) {
                    berryID = v0;
                    direction++;
        ...
        if (berryID != NUM_BERRIES - 1) {
            while (TRUE) {
                ...
                if (BerryTagData_Contains(entries, v0) == 1) {
                    berryID = (u16)v0;
                    direction--;
