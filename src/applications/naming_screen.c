    ...
    String *string)
{
    ...
    case NMS_BUTTON_OK:
        charCode = CHAR_JP_SPACE;
        break;
    }

    for (i = 0; i < 3; i++) {
        charCodeBuf[i] = CHAR_JP_SPACE;
    }

    charCodeBuf[0] = charCode;

    if (charCode != CHAR_JP_SPACE) {
...
static int NamingScreen_ProcessCharacterInput(NamingScreen *namingScreen, charcode_t charCode, BOOL isButtonInput)
{
    ...
    case NMS_BUTTON_OK:
        ...
        break;
    default:
        if (namingScreen->currentCharsIdx == 4 && charCode == CHAR_JP_SPACE) {
            return NMS_APP_STATE_RUNNING;
        }
...
static BOOL NamingScreen_JPFlipAlphaCase(int tableStart, int tableEnd, charcode_t *charBuf, int cursorPos)
{
    ...
    for (i = tableStart; i < tableEnd; i++) {
        for (j = 0; j < 3; j++) {
            if (sJPCharConversionTable[i][j] == charCode && charCode != CHAR_JP_SPACE) {
    ...
    int cursorPos)
{
    ...
    if (cursorPos == 0 || charBuf[cursorPos - 1] == CHAR_JP_SPACE) {
        return FALSE;
    }
    ...
    case NMS_CONTROL_HANDAKU:
        ...
        break;
    case NMS_BUTTON_PAGE_JP_UNUSED_2:
        if (charCode == CHAR_JP_HIRA_DZU) {
            charBuf[cursorPos - 1] = CHAR_JP_HIRA_SOKUON;
            return TRUE;
        }

        if (charCode == CHAR_JP_KATA_DZU) {
            charBuf[cursorPos - 1] = CHAR_JP_KATA_SOKUON;
            return TRUE;
        }
        break;
    }
