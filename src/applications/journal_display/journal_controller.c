...
int JournalController_Init(OverlayManager *overlayMan, int *state)
{
    ...
    saveData = OverlayManager_Args(overlayMan);
    journalManager = OverlayManager_NewData(overlayMan, sizeof(JournalManager), HEAP_ID_JOURNAL);
...
{
    JournalManager *journalManager = OverlayManager_Data(overlayMan);
