...
static void PCHallOfFame_DrawAllPokemon(PCHallOfFameApp *pcHallOfFameApp)
{
    ...
    for (i = 0; i < pcHallOfFameScreen->pokemonCount; i++) {
        Pokemon_InitWithParams(pcHallOfFameApp->mon, pcHallOfFameScreen->pokemon[i].species, pcHallOfFameScreen->pokemon[i].level, INIT_IVS_RANDOM, TRUE, pcHallOfFameScreen->pokemon[i].personality, OT_ID_PRESET, pcHallOfFameScreen->pokemon[i].OTID);
