...
static void GetPartyMonInfo(PartyStatus *data, Party *party)
{
    ...
    for (int i = 0; i < data->partyCount; i++) {
        ...
        decrypted = Pokemon_UnlockEncryption(mon);
        ...
        data->mons[i].currentHp = Pokemon_GetData(mon, MON_DATA_HP, NULL);
