...
static const TouchScreenRect screenHitBox[] = {
    { .rect = {
...
static BOOL Init(PoketchAnalogWatch *appData, PoketchSystem *poketchSys, BgConfig *bgConfig, u32 appID)
{
    if (PoketchAnalogWatch_New(&appData->graphics, &appData->watchData, bgConfig)) {
        ...
        appData->buttonManager = PoketchButtonManager_New(screenHitBox, NELEMS(screenHitBox), TouchCallback, appData, HEAP_ID_POKETCH_APP);
