...
{
    u16 *approachType = FieldSystem_GetScriptMemberPtr(ctx->fieldSystem, SCRIPT_MANAGER_TRAINER_0_TYPE);
    u16 *destVar = ScriptContext_GetVarPointer(ctx);

    *destVar = *approachType;
    return FALSE;
}
...
BOOL ScrCmd_StartDummyTrainerBattle(ScriptContext *ctx)
{
    FieldSystem *fieldSystem = ctx->fieldSystem;
    int *battleResultMaskPtr = FieldSystem_GetScriptMemberPtr(fieldSystem, SCRIPT_MANAGER_BATTLE_RESULT);
    Encounter_NewVsTrainer(ctx->task, TRAINER_YOUNGSTER_TRISTAN, TRAINER_NONE, TRAINER_NONE, HEAP_ID_FIELD2, battleResultMaskPtr);
    return TRUE;
}
