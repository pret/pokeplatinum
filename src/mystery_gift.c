...
{
    if ((type > MYST_GIFT_NONE) && (type < MYST_GIFT_TYPE_MAX)) {
        return TRUE;
    }
...
static PGT *MysteryGift_TryGetPgt(MysteryGift *mysteryGift, int pgtSlot)
{
    if ((pgtSlot >= 0) && (pgtSlot < NUM_PGT_SLOTS)) {
...
{
    if ((wcSlot >= 0) && (wcSlot < NUM_WONDERCARD_SLOTS)) {
        if (CheckIsValidWcType(mysteryGift->wonderCards[wcSlot].pgt.type)) {
...
BOOL MysteryGift_FreeWcErasePgt(MysteryGift *mysteryGift, int pgtSlot)
{
    GF_ASSERT(pgtSlot < NUM_WONDERCARD_SLOTS);

    mysteryGift->wonderCards[pgtSlot].pgt.type = MYST_GIFT_NONE;

    MysteryGift_UnsetWcIDReceived(mysteryGift, mysteryGift->wonderCards[pgtSlot].metadata.id);
    MysteryGift_FreePgtSlotForWc(mysteryGift, pgtSlot);
    SaveData_SetChecksum(SAVE_TABLE_ENTRY_MYSTERY_GIFT);
...
BOOL MysteryGift_CheckWcHasPgtSaved(const MysteryGift *mysteryGift, int wondercardSlot)
{
    for (int pgtSlot = 0; pgtSlot < NUM_PGT_SLOTS; pgtSlot++) {
        if (CheckIsValidWcType(mysteryGift->pgts[pgtSlot].type)) {
            if (mysteryGift->pgts[pgtSlot].wondercardSlot == wondercardSlot) {
                return TRUE;
            }
        }
    }
...
void MysteryGift_Load(SaveData *saveData, int unused)
{
    if (sMysteryGift == NULL) {
        sMysteryGift = SaveData_MysteryGift(saveData);
    }
