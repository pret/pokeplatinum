...
static void EndVoiceChatSession(VCTSession *param0);
...
{
#pragma unused(unused0, unused1)
    return;
}

static void SoundsStreamCallback(NNSSndStrmCallbackStatus status, int numOtherClients, void *receiveBuffers[], u32 bufferSize, NNSSndStrmFormat unused, void *_sendBuffer)
{
...
static int AcceptIncomingCall(u8 unused)
{
    ...
    if ((sVoiceChatManager->session != NULL) && (sVoiceChatManager->session->state == VCT_STATE_INCOMING)) {
...
static void Alloc32ByteAligned(void **allocStartPtr, void **alignedPtr, int size, int heapID)
{
    *allocStartPtr = Heap_Alloc(heapID, size + 32);
    MI_CpuClear8(*allocStartPtr, size + 32);
    *alignedPtr = (VoiceChatManager *)(((u32)*allocStartPtr + 31) / 32 * 32);
}
...
void VoiceChat_Start(enum HeapID heapID, int codec, int numConferenceSessions)
{
    ...
    NNS_SndStrmSetup(&sVoiceChatManager->soundStream, NNS_SND_STRM_FORMAT_PCM16, sVoiceChatManager->receiveBuffer, VOICE_CHAT_BUFFER_SIZE, NNS_SND_STRM_TIMER_CLOCK / 8000, 2, SoundsStreamCallback, sVoiceChatManager->sendBuffer);
...
void VoiceChat_RequestEnd(void)
{
    ...
    if ((sVoiceChatManager->session == NULL) || (sVoiceChatManager->state == VOICE_CHAT_STATE_INIT)) {
        NintendoWFC_TerminateVoiceChat();
        return;
    }
