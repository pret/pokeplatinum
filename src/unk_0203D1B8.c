...
{
    AccessoryShop *accessoryShop = FieldTask_GetEnv(task);

    if (AccessoryShop_Main(accessoryShop) == TRUE) {
        AccessoryShop_Free(accessoryShop);
        return TRUE;
    }
...
void AccessoryShop_Init(FieldTask *task)
{
    ...
    AccessoryShop *accessoryShop = AccessoryShop_Alloc(HEAP_ID_FIELD, fieldSystem->saveData, fieldSystem->bgConfig);

    FieldTask_InitCall(task, FieldTask_AccessoryShop, accessoryShop);
}
