...
typedef struct TrapsEnv {
    ...
    UnkStruct_ov101_021D5D90 *unk_368[MAX_PLACED_TRAPS];
...
static UnkStruct_ov101_021D5D90 *UndergroundTraps_DrawPlacedTrap(int x, int z, int size, int trapID);
...
void TrapsEnv_Free(void)
{
    for (int i = 0; i < MAX_PLACED_TRAPS; i++) {
        if (trapsEnv->unk_368[i]) {
            sub_0207136C(trapsEnv->unk_368[i]);
            trapsEnv->unk_368[i] = NULL;
        }
    }
...
static void UndergroundTraps_RemovePlacedTrapCurrentPlayer(BuriedTrap *trap)
{
    ...
    if (trapsEnv->unk_368[index] != NULL) {
        sub_0207136C(trapsEnv->unk_368[index]);
    }
...
{
    if (!trapsEnv->graphicsDisabled) {
        UnkStruct_ov101_021D5D90 *v0 = ov5_DrawFloorTexture(trapsEnv->fieldSystem, x, z, size, trapID - 1);
        return v0;
    }
