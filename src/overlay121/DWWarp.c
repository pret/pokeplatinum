...
typedef struct {
    ...
    int frame_cnt;
    int soundEffect_cnt;
    UnkStruct_02017294 animation_obj;
    UnkStruct_02017248 animation_animation;
    UnkStruct_02017248 animation_animation2;
    UnkStruct_020170F4 animation_model;
    u32 move_frame;
    int camera_angle;
    int camera_angle_calc;
    fx32 camera_distance;
    int camera_perspective;
    NNSFndAllocator allocator;
} DistortionWorld_Warp;

static void DW_Update(SysTask * task, void * data);
static void DW_VBlankIntr(void * data);
static void DW_VramSetBank(void);
static void DW_InitCamera(DistortionWorld_Warp * warp);
static void DWWarp_CameraEnd(DistortionWorld_Warp * warp);
static void DWWarp_InitModel(DistortionWorld_Warp * warp);
static void DWWarp_DeleteModel(DistortionWorld_Warp * warp);
static void Model3D_Update(DistortionWorld_Warp * warp);
...
static void DWWarp_CameraMove(DistortionWorld_Warp * warp);

// Initializes the Distortion World warp effect overlay.
int DWWarp_Init (OverlayManager * ovy, int * seq)
{
    DistortionWorld_Warp * dww;
    ...
    dww = OverlayManager_NewData(ovy, sizeof(DistortionWorld_Warp), 30);
    MI_CpuClear8(dww, sizeof(DistortionWorld_Warp));
    dww->p3DCallback = DWWarp_Init3D(30);
...
{
    DistortionWorld_Warp * warp = OverlayManager_Data(ovy);
    ...
    case DWARP_SEQ_LOOP:
        ...
        if (warp->soundEffect_cnt == 15) {
            Sound_PlayEffect(1491);
        }

        warp->soundEffect_cnt++;
        warp->frame_cnt++;

        // Continue to the next sequence after 85 frames
        if (warp->frame_cnt > 85) {
            (*seq)++;
        }
        break;
...
static void DWWarp_InitModel (DistortionWorld_Warp * warp)
{
    ...
    Heap_FndInitAllocatorForExpHeap(&warp->allocator, 30, 4);

    narc = NARC_ctor(NARC_INDEX_DEMO__TITLE__TITLEDEMO, 30);

    {
        sub_020170D8(&warp->animation_model, narc, 16, 30);

        NNS_G3dMdlUseMdlAlpha(warp->animation_model.unk_08);
        NNS_G3dMdlUseMdlPolygonID(warp->animation_model.unk_08);

        sub_02017164(&warp->animation_animation, &warp->animation_model, narc, 18, 30, &warp->allocator);
        sub_02017240(&warp->animation_animation, 0);

        sub_02017164(&warp->animation_animation2, &warp->animation_model, narc, 17, 30, &warp->allocator);
        sub_02017240(&warp->animation_animation2, 0);
        sub_02017258(&warp->animation_obj, &warp->animation_model);

        sub_02017350(&warp->animation_obj, 0, 0, 0);
        sub_0201736C(&warp->animation_obj, (FX32_ONE), (FX32_ONE), (FX32_ONE));
        sub_02017348(&warp->animation_obj, 1);

        sub_0201727C(&warp->animation_obj, &warp->animation_animation);
        sub_0201727C(&warp->animation_obj, &warp->animation_animation2);
    }

    NARC_dtor(narc);

    warp->camera_angle = ((0x10000 - 0x3fef) - (0x10000 - 0x1c7d)) / 30;
    warp->camera_angle_calc = (0x10000 - 0x1c7d);
    warp->camera_distance = (FX32_ONE);
    warp->camera_perspective = (60 << 8);
}

static void DWWarp_DeleteModel (DistortionWorld_Warp * warp)
{
    sub_02017110(&warp->animation_model);
    sub_020171A0(&warp->animation_animation, &warp->allocator);
    sub_020171A0(&warp->animation_animation2, &warp->allocator);
}
...
static void Model3D_Update (DistortionWorld_Warp * warp)
{
    ...
    sub_020171CC(&warp->animation_animation, FX32_ONE);
    sub_020171CC(&warp->animation_animation2, FX32_ONE);

    NNS_G3dGePushMtx();

    {
        sub_02017294(&warp->animation_obj);
    }
...
static GenericPointerData * DWWarp_Init3D (int param0)
{
    ...
    v0 = sub_02024220(param0, 0, 2, 0, 2, DWWarp_Setup3D);
    return v0;
}
...
static void DWWarp_CameraMove (DistortionWorld_Warp * warp)
{
    ...
    sub_0202094C(-(warp->camera_perspective >> 8), warp->camera);
    warp->camera_perspective -= 0x80;

    if (warp->camera_perspective < (16 << 8)) {
