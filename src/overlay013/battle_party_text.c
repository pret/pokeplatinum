...
static void PrintPokemonNameHeader(BattleParty *battleParty, u32 windowIndex, enum Font font, u16 partyIndex, u8 xOffset, u8 yOffset)
{
    ...
    String *formattedStrbuf = String_Init(12, battleParty->context->heapID);
    String *strbuf = MessageLoader_GetNewString(battleParty->messageLoader, sPartyPokemonNameTextIDs[partyIndex]);
    ...
    String_Free(strbuf);
    String_Free(formattedStrbuf);

    if (pokemon->displayNidoranGender == FALSE && pokemon->isEgg == FALSE) {
        u32 genderIconXOffset;
        if (pokemon->gender == GENDER_MALE) {
            strbuf = MessageLoader_GetNewString(battleParty->messageLoader, BattleParty_Text_MaleIcon);
            genderIconXOffset = Window_GetWidth(window) * 8 - Font_CalcStringWidth(FONT_SYSTEM, strbuf, 0);
            ...
            strbuf = MessageLoader_GetNewString(battleParty->messageLoader, BattleParty_Text_FemaleIcon);
            genderIconXOffset = Window_GetWidth(window) * 8 - Font_CalcStringWidth(FONT_SYSTEM, strbuf, 0);
...
{
    BattlePartyPokemon *pokemon = &battleParty->partyPokemon[partyIndex];
    String *formattedStrbuf = String_Init(16, battleParty->context->heapID);
    String *strbuf = MessageLoader_GetNewString(battleParty->messageLoader, BattleParty_Text_PokemonAbilityName);
...
static void PrintPokemonHeldItem(BattleParty *battleParty, u32 windowIndex, u32 partyIndex)
{
    ...
    if (pokemon->heldItem == ITEM_NONE) {
        formattedStrbuf = MessageLoader_GetNewString(battleParty->messageLoader, BattleParty_Text_PokemonNoItemHeld);
    } else {
        formattedStrbuf = String_Init(18, battleParty->context->heapID);
        strbuf = MessageLoader_GetNewString(battleParty->messageLoader, BattleParty_Text_PokemonHeldItemName);
...
static void PrintMoveName(BattleParty *battleParty, enum Move move, u32 windowIndex, u32 textID, u16 font, u16 yOffset, TextColor color)
{
    Window *window = &battleParty->windows[windowIndex];
    String *formattedStrbuf = String_Init(16, battleParty->context->heapID);
    String *strbuf = MessageLoader_GetNewString(battleParty->messageLoader, textID);
    u32 xOffset;
    ...
    if (font == FONT_SUBSCREEN) {
        xOffset = (Window_GetWidth(window) * 8 - Font_CalcStringWidth(font, formattedStrbuf, 0)) / 2;
    } else {
        xOffset = 0;
    }
...
{
    String *strbuf = MessageLoader_GetNewString(battleParty->messageLoader, BattleParty_Text_PokemonPPLabel);
...
static void PrintSelectOptionText(BattleParty *battleParty, u32 textID)
{
    ...
    strbuf = MessageLoader_GetNewString(battleParty->messageLoader, textID);
...
static void PrintButtonText(BattleParty *battleParty, u32 windowIndex, u32 textID)
{
    Window *window = &battleParty->windows[windowIndex];
    String *strbuf = MessageLoader_GetNewString(battleParty->messageLoader, textID);
    u32 stringWidth = Font_CalcStringWidth(FONT_SUBSCREEN, strbuf, 0);
...
static void PrintSelectedPokemonLevel(BattleParty *battleParty, u32 partyIndex)
{
    ...
    String *strbuf = MessageLoader_GetNewString(battleParty->messageLoader, BattleParty_Text_PokemonLevelLabel);
    String *formattedStrbuf;
    u16 xOffset;
    ...
    strbuf = MessageLoader_GetNewString(battleParty->messageLoader, BattleParty_Text_PokemonLevelValue);
    formattedStrbuf = String_Init(8, battleParty->context->heapID);
    ...
    strbuf = MessageLoader_GetNewString(battleParty->messageLoader, BattleParty_Text_PokemonExpToNextLevelLabel);
    ...
    strbuf = MessageLoader_GetNewString(battleParty->messageLoader, BattleParty_Text_PokemonExpToNextLevelValue);
    formattedStrbuf = String_Init(14, battleParty->context->heapID);
    ...
    xOffset = Window_GetWidth(&battleParty->windows[POKEMON_SUMMARY_SCREEN_WINDOW_EXP_TO_NEXT_LVL + windowIndexOffset]) * 8 - Font_CalcStringWidth(FONT_SYSTEM, formattedStrbuf, 0);
...
static void PrintPokemonAttackStat(BattleParty *battleParty, u32 partyIndex)
{
    BattlePartyPokemon *pokemon = &battleParty->partyPokemon[partyIndex];
    u16 windowIndexOffset = POKEMON_SUMMARY_SCREEN_WINDOW_ALT_POKEMON_NAME * battleParty->useAltSummaryWindows;
    String *strbuf = MessageLoader_GetNewString(battleParty->messageLoader, BattleParty_Text_PokemonAttackStatLabel);
    String *formattedStrbuf;
    u8 stringWidth;
    u8 xOffset;
    ...
    strbuf = MessageLoader_GetNewString(battleParty->messageLoader, BattleParty_Text_PokemonAttackStatValue);
    formattedStrbuf = String_Init(8, battleParty->context->heapID);
    ...
    stringWidth = Font_CalcStringWidth(FONT_SYSTEM, formattedStrbuf, 0);
...
static void PrintPokemonDefenseStat(BattleParty *battleParty, u32 partyIndex)
{
    BattlePartyPokemon *pokemon = &battleParty->partyPokemon[partyIndex];
    u16 windowIndexOffset = POKEMON_SUMMARY_SCREEN_WINDOW_ALT_POKEMON_NAME * battleParty->useAltSummaryWindows;
    String *strbuf = MessageLoader_GetNewString(battleParty->messageLoader, BattleParty_Text_PokemonDefenseStatLabel);
    String *formattedStrbuf;
    u8 stringWidth;
    u8 xOffset;
    ...
    strbuf = MessageLoader_GetNewString(battleParty->messageLoader, BattleParty_Text_PokemonDefenseStatValue);
    formattedStrbuf = String_Init(8, battleParty->context->heapID);
...
static void PrintPokemonSpeedStat(BattleParty *battleParty, u32 partyIndex)
{
    BattlePartyPokemon *pokemon = &battleParty->partyPokemon[partyIndex];
    u16 windowIndexOffset = POKEMON_SUMMARY_SCREEN_WINDOW_ALT_POKEMON_NAME * battleParty->useAltSummaryWindows;
    String *strbuf = MessageLoader_GetNewString(battleParty->messageLoader, BattleParty_Text_PokemonSpeedStatLabel);
    String *formattedStrbuf;
    u8 stringWidth;
    u8 xOffset;
    ...
    strbuf = MessageLoader_GetNewString(battleParty->messageLoader, BattleParty_Text_PokemonSpeedStatValue);
    formattedStrbuf = String_Init(8, battleParty->context->heapID);
...
static void PrintPokemonSpAtkStat(BattleParty *battleParty, u32 partyIndex)
{
    BattlePartyPokemon *pokemon = &battleParty->partyPokemon[partyIndex];
    u16 windowIndexOffset = POKEMON_SUMMARY_SCREEN_WINDOW_ALT_POKEMON_NAME * battleParty->useAltSummaryWindows;
    String *strbuf = MessageLoader_GetNewString(battleParty->messageLoader, BattleParty_Text_PokemonSpAtkStatLabel);
    String *formattedStrbuf;
    u8 stringWidth;
    u8 xOffset;
    ...
    strbuf = MessageLoader_GetNewString(battleParty->messageLoader, BattleParty_Text_PokemonSpAtkStatValue);
    formattedStrbuf = String_Init(8, battleParty->context->heapID);
...
static void PrintPokemonSpDefStat(BattleParty *battleParty, u32 partyIndex)
{
    BattlePartyPokemon *pokemon = &battleParty->partyPokemon[partyIndex];
    u16 windowIndexOffset = POKEMON_SUMMARY_SCREEN_WINDOW_ALT_POKEMON_NAME * battleParty->useAltSummaryWindows;
    String *strbuf = MessageLoader_GetNewString(battleParty->messageLoader, BattleParty_Text_PokemonSpDefStatLabel);
    String *formattedStrbuf;
    u8 stringWidth;
    u8 xOffset;
    ...
    strbuf = MessageLoader_GetNewString(battleParty->messageLoader, BattleParty_Text_PokemonSpDefStatValue);
    formattedStrbuf = String_Init(8, battleParty->context->heapID);
...
static void PrintPokemonHPStat(BattleParty *battleParty, u32 partyIndex)
{
    BattlePartyPokemon *pokemon = &battleParty->partyPokemon[partyIndex];
    u16 windowIndexOffset = POKEMON_SUMMARY_SCREEN_WINDOW_ALT_POKEMON_NAME * battleParty->useAltSummaryWindows;
    String *strbuf = MessageLoader_GetNewString(battleParty->messageLoader, BattleParty_Text_PokemonHPStatLabel);
    String *formattedStrbuf;
    ...
    strbuf = MessageLoader_GetNewString(battleParty->messageLoader, BattleParty_Text_PokemonHPStatDivider);
    stringWidth = Font_CalcStringWidth(FONT_SYSTEM, strbuf, 0);
    ...
    strbuf = MessageLoader_GetNewString(battleParty->messageLoader, BattleParty_Text_PokemonHPStatValue);
    formattedStrbuf = String_Init(8, battleParty->context->heapID);
    ...
    formattedStringWidth = Font_CalcStringWidth(FONT_SYSTEM, formattedStrbuf, 0);
    ...
    strbuf = MessageLoader_GetNewString(battleParty->messageLoader, BattleParty_Text_PokemonMaxHPStatValue);
    formattedStrbuf = String_Init(8, battleParty->context->heapID);
...
static void PrintPokemonAbilityDescription(BattleParty *battleParty, u32 partyIndex)
{
    ...
    String *strbuf = MessageLoader_GetNewString(messageLoader, pokemon->ability);
...
{
    Window *window = &battleParty->windows[windowIndex];
    String *strbuf = MessageLoader_GetNewString(battleParty->messageLoader, BattleParty_Text_MoveAccuracyLabel);
...
static void PrintMoveAccuracyValue(BattleParty *battleParty, u32 windowIndex, u32 moveAccuracyStat)
{
    ...
    if (moveAccuracyStat == 0) {
        strbuf = MessageLoader_GetNewString(battleParty->messageLoader, BattleParty_Text_MoveStatNoValue);
        stringWidth = Font_CalcStringWidth(FONT_SYSTEM, strbuf, 0);
        ...
        strbuf = MessageLoader_GetNewString(battleParty->messageLoader, BattleParty_Text_MoveAccuracyValue);
        formattedStrbuf = String_Init(8, battleParty->context->heapID);
...
{
    Window *window = &battleParty->windows[windowIndex];
    String *strbuf = MessageLoader_GetNewString(battleParty->messageLoader, BattleParty_Text_MovePowerLabel);
...
static void PrintMovePowerValue(BattleParty *battleParty, u32 windowIndex, u32 movePowerStat)
{
    ...
    if (movePowerStat <= 1) {
        ...
        strbuf = MessageLoader_GetNewString(battleParty->messageLoader, BattleParty_Text_MovePowerValue);
        formattedStrbuf = String_Init(8, battleParty->context->heapID);
...
static void PrintMoveDescription(BattleParty *battleParty, u32 windowIndex, u32 textID)
{
    ...
    String *strbuf = MessageLoader_GetNewString(messageLoader, textID);
...
{
    Window *window = &battleParty->windows[windowIndex];
    String *strbuf = MessageLoader_GetNewString(battleParty->messageLoader, BattleParty_Text_MoveCategoryLabel);
    u16 stringWidth = Font_CalcStringWidth(FONT_SYSTEM, strbuf, 0);
...
static void PrintMoveCategory(BattleParty *battleParty, u32 windowIndex, u32 moveCategory)
{
    ...
    case BATTLE_PARTY_POKEMON_MOVE_CATEGORY_PHYSICAL:
        strbuf = MessageLoader_GetNewString(battleParty->messageLoader, BattleParty_Text_MoveCategoryPhysical);
        break;
    case BATTLE_PARTY_POKEMON_MOVE_CATEGORY_SPECIAL:
        strbuf = MessageLoader_GetNewString(battleParty->messageLoader, BattleParty_Text_MoveCategorySpecial);
        break;
    case BATTLE_PARTY_POKEMON_MOVE_CATEGORY_STATUS:
        strbuf = MessageLoader_GetNewString(battleParty->messageLoader, BattleParty_Text_MoveCategoryStatus);
    }
...
{
    Window *window = &battleParty->windows[windowIndex];
    String *strbuf = MessageLoader_GetNewString(battleParty->messageLoader, BattleParty_Text_MovePPDivider);
    String *formattedStrbuf;
    u32 stringWidth, formattedStringWidth;
    u32 xOffset;
    ...
    strbuf = MessageLoader_GetNewString(battleParty->messageLoader, BattleParty_Text_MoveCurrentPP);
    formattedStrbuf = String_Init(6, battleParty->context->heapID);
    ...
    strbuf = MessageLoader_GetNewString(battleParty->messageLoader, BattleParty_Text_MoveMaxPP);
    formattedStrbuf = String_Init(6, battleParty->context->heapID);
...
static void PrintConfirmMoveButton(BattleParty *battleParty, u32 windowIndex)
{
    ...
    if (battleParty->context->selectedMoveSlot == MOVE_TO_LEARN_SLOT) {
        strbuf = MessageLoader_GetNewString(battleParty->messageLoader, BattleParty_Text_CancelMoveButton);
    } else {
        strbuf = MessageLoader_GetNewString(battleParty->messageLoader, BattleParty_Text_ForgetMoveButton);
    }

    stringWidth = Font_CalcStringWidth(FONT_SUBSCREEN, strbuf, 0);
...
void BattlePartyText_PrintHMMovesCantBeForgottenText(BattleParty *battleParty)
{
    ...
    strbuf = MessageLoader_GetNewString(battleParty->messageLoader, BattleParty_Text_HMMovesCantBeForgotten);
...
{
    Window *window = &battleParty->windows[windowIndex];
    String *formattedStrbuf = String_Init(6, battleParty->context->heapID);
    String *strbuf = MessageLoader_GetNewString(battleParty->messageLoader, BattleParty_Text_MovePPLabel);
    u32 stringWidth;

    Text_AddPrinterWithParamsAndColor(window, FONT_SYSTEM, strbuf, 40, 24, TEXT_SPEED_NO_TRANSFER, TEXT_COLOR(15, 14, 0), NULL);
    String_Free(strbuf);

    strbuf = MessageLoader_GetNewString(battleParty->messageLoader, BattleParty_Text_MovePPDivider);
    stringWidth = Font_CalcStringWidth(FONT_SYSTEM, strbuf, 0);
...
static void PrintMoveCurrentPP(BattleParty *battleParty, BattlePartyPokemonMove *move, u32 windowIndex)
{
    ...
    String *formattedStrbuf = String_Init(6, battleParty->context->heapID);
    String *strbuf = MessageLoader_GetNewString(battleParty->messageLoader, BattleParty_Text_MovePPLabel);
    u32 stringWidth = Font_CalcStringWidth(FONT_SYSTEM, strbuf, 0);
...
static void PrintSelectedPokemonName(BattleParty *battleParty, u32 partyIndex)
{
    ...
    BattlePartyPokemon *pokemon = &battleParty->partyPokemon[partyIndex];
    String *strbuf = String_Init(12, battleParty->context->heapID);
    String *formattedStrbuf = MessageLoader_GetNewString(battleParty->messageLoader, sPartyPokemonNameTextIDs[partyIndex]);
    u8 stringWidth;
    ...
    if (pokemon->displayNidoranGender == FALSE && pokemon->isEgg == FALSE) {
        if (pokemon->gender == GENDER_MALE) {
            formattedStrbuf = MessageLoader_GetNewString(battleParty->messageLoader, BattleParty_Text_MaleIcon);
        } else if (pokemon->gender == GENDER_FEMALE) {
            formattedStrbuf = MessageLoader_GetNewString(battleParty->messageLoader, BattleParty_Text_FemaleIcon);
        }
    }
...
static void RenderConfirmLearnMoveContestStatsScreen(BattleParty *battleParty)
{
    ...
    strbuf = MessageLoader_GetNewString(battleParty->messageLoader, BattleParty_Text_AppealPtsLabel);
...
void BattlePartyText_PrintUseItemEffect(BattleParty *battleParty)
{
    ...
    if (battleParty->partyPokemon[context->selectedPartyIndex].curHP == 0 && newHP != 0) {
        strbuf = MessageLoader_GetNewString(battleParty->messageLoader, BattleParty_Text_ItemRevivedPokemon);
        StringTemplate_SetNickname(battleParty->stringTemplate, 0, Pokemon_GetBoxPokemon(pokemon));
        StringTemplate_Format(battleParty->stringTemplate, battleParty->string, strbuf);
        String_Free(strbuf);
    } else if (battleParty->partyPokemon[context->selectedPartyIndex].curHP != newHP) {
        strbuf = MessageLoader_GetNewString(battleParty->messageLoader, BattleParty_Text_ItemRestoredHealth);
        StringTemplate_SetNickname(battleParty->stringTemplate, 0, Pokemon_GetBoxPokemon(pokemon));
        ...
        strbuf = MessageLoader_GetNewString(battleParty->messageLoader, BattleParty_Text_ItemCuredSleep);
        StringTemplate_SetNickname(battleParty->stringTemplate, 0, Pokemon_GetBoxPokemon(pokemon));
        StringTemplate_Format(battleParty->stringTemplate, battleParty->string, strbuf);
        String_Free(strbuf);
    } else if (healedStatusEffectFlags == HEALED_STATUS_EFFECT_FLAG_POISON) {
        strbuf = MessageLoader_GetNewString(battleParty->messageLoader, BattleParty_Text_ItemCuredPoisoning);
        StringTemplate_SetNickname(battleParty->stringTemplate, 0, Pokemon_GetBoxPokemon(pokemon));
        StringTemplate_Format(battleParty->stringTemplate, battleParty->string, strbuf);
        String_Free(strbuf);
    } else if (healedStatusEffectFlags == HEALED_STATUS_EFFECT_FLAG_BURN) {
        strbuf = MessageLoader_GetNewString(battleParty->messageLoader, BattleParty_Text_ItemCuredBurn);
        StringTemplate_SetNickname(battleParty->stringTemplate, 0, Pokemon_GetBoxPokemon(pokemon));
        StringTemplate_Format(battleParty->stringTemplate, battleParty->string, strbuf);
        String_Free(strbuf);
    } else if (healedStatusEffectFlags == HEALED_STATUS_EFFECT_FLAG_FREEZE) {
        strbuf = MessageLoader_GetNewString(battleParty->messageLoader, BattleParty_Text_ItemCuredFreeze);
        StringTemplate_SetNickname(battleParty->stringTemplate, 0, Pokemon_GetBoxPokemon(pokemon));
        StringTemplate_Format(battleParty->stringTemplate, battleParty->string, strbuf);
        String_Free(strbuf);
    } else if (healedStatusEffectFlags == HEALED_STATUS_EFFECT_FLAG_PARALYSIS) {
        strbuf = MessageLoader_GetNewString(battleParty->messageLoader, BattleParty_Text_ItemCuredParalysis);
        StringTemplate_SetNickname(battleParty->stringTemplate, 0, Pokemon_GetBoxPokemon(pokemon));
        StringTemplate_Format(battleParty->stringTemplate, battleParty->string, strbuf);
        String_Free(strbuf);
    } else if (healedStatusEffectFlags == HEALED_STATUS_EFFECT_FLAG_CONFUSION) {
        strbuf = MessageLoader_GetNewString(battleParty->messageLoader, BattleParty_Text_ItemCuredConfusion);
        StringTemplate_SetNickname(battleParty->stringTemplate, 0, Pokemon_GetBoxPokemon(pokemon));
        StringTemplate_Format(battleParty->stringTemplate, battleParty->string, strbuf);
        String_Free(strbuf);
    } else if (healedStatusEffectFlags == HEALED_STATUS_EFFECT_FLAG_INFATUATION) {
        strbuf = MessageLoader_GetNewString(battleParty->messageLoader, BattleParty_Text_ItemCuredInfatuation);
        StringTemplate_SetNickname(battleParty->stringTemplate, 0, Pokemon_GetBoxPokemon(pokemon));
        StringTemplate_Format(battleParty->stringTemplate, battleParty->string, strbuf);
        String_Free(strbuf);
    } else {
        strbuf = MessageLoader_GetNewString(battleParty->messageLoader, BattleParty_Text_ItemCuredMultipleStatuses);
        StringTemplate_SetNickname(battleParty->stringTemplate, 0, Pokemon_GetBoxPokemon(pokemon));
        StringTemplate_Format(battleParty->stringTemplate, battleParty->string, strbuf);
        String_Free(strbuf);
    }
...
void BattlePartyText_PrintEmbargoPreventingItemUse(BattleParty *battleParty)
{
    ...
    String *strbuf = MessageLoader_GetNewString(battleParty->messageLoader, BattleParty_Text_EmbargoPreventsItemUse);
