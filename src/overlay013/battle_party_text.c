...
#define BATTLE_PARTY_POKEMON_PARTY_SCREEN_WINDOW_NUM                    6
...
static const WindowTemplate sPartyPokemonScreenWindowTemplates[] = {
    [BATTLE_PARTY_POKEMON_PARTY_SCREEN_WINDOW_PARTY_SLOT_1] = {
...
static const WindowTemplate sSelectPokemonScreenWindowTemplates[] = {
    [BATTLE_PARTY_SELECT_POKEMON_SCREEN_WINDOW_POKEMON_NAME] = {
        .bgLayer = BG_LAYER_SUB_1,
        .tilemapLeft = 10,
        .tilemapTop = 4,
        .width = 12,
...
static const WindowTemplate sPokemonSummaryScreenWindowTemplates[] = {
    [BATTLE_PARTY_POKEMON_SUMMARY_SCREEN_WINDOW_POKEMON_NAME] = {
        .bgLayer = BG_LAYER_SUB_1,
        .tilemapLeft = 5,
        .tilemapTop = 1,
        .width = 9,
...
static const WindowTemplate sPokemonMovesScreenWindowTemplates[] = {
    [BATTLE_PARTY_POKEMON_MOVES_SCREEN_WINDOW_POKEMON_NAME] = {
        .bgLayer = BG_LAYER_SUB_1,
        .tilemapLeft = 5,
        .tilemapTop = 1,
        .width = 9,
...
static const WindowTemplate sMoveSummaryScreenWindowTemplates[] = {
    [BATTLE_PARTY_MOVE_SUMMARY_SCREEN_WINDOW_MOVE_NAME] = {
...
static const WindowTemplate sRestoreMovePPScreenWindowTemplates[] = {
    [BATTLE_PARTY_RESTORE_MOVE_PP_SCREEN_WINDOW_POKEMON_NAME] = {
        .bgLayer = BG_LAYER_SUB_0,
        .tilemapLeft = 5,
        .tilemapTop = 1,
        .width = 9,
...
static const WindowTemplate sLearnMoveScreenWindowTemplates[] = {
    [BATTLE_PARTY_LEARN_MOVE_SCREEN_WINDOW_POKEMON_NAME] = {
        .bgLayer = BG_LAYER_SUB_0,
        .tilemapLeft = 5,
        .tilemapTop = 1,
        .width = 9,
...
static const WindowTemplate sConfirmLearnMoveScreenWindowTemplates[] = {
    [BATTLE_PARTY_CONFIRM_LEARN_MOVE_SCREEN_WINDOW_POKEMON_NAME] = {
        .bgLayer = BG_LAYER_SUB_1,
        .tilemapLeft = 5,
        .tilemapTop = 1,
        .width = 9,
...
static const WindowTemplate sConfirmLearnMoveContestStatsWindowTemplates[] = {
    [BATTLE_PARTY_CONFIRM_LEARN_MOVE_CONTEST_STATS_SCREEN_WINDOW_POKEMON_NAME] = {
        .bgLayer = BG_LAYER_SUB_1,
        .tilemapLeft = 5,
        .tilemapTop = 1,
        .width = 9,
...
void BattlePartyText_InitializeScreenWindows(BattleParty *battleParty, enum BattlePartyScreen screen)
{
    ...
    case BATTLE_PARTY_SCREEN_POKEMON_PARTY:
        ...
        battleParty->numWindows = BATTLE_PARTY_POKEMON_PARTY_SCREEN_WINDOW_NUM;
        break;
...
{
    BattlePartyPokemon *pokemon = &battleParty->partyPokemon[partyIndex];
    u16 windowIndexOffset = BATTLE_PARTY_POKEMON_SUMMARY_SCREEN_WINDOW_ALT_POKEMON_NAME * battleParty->useAltSummaryWindows;
    Strbuf *strbuf = MessageLoader_GetNewStrbuf(battleParty->messageLoader, BattleParty_Text_PokemonLevelLabel);
    Strbuf *formattedStrbuf;
    u16 xOffset;

    Text_AddPrinterWithParamsAndColor(&battleParty->windows[BATTLE_PARTY_POKEMON_SUMMARY_SCREEN_WINDOW_LEVEL_LABEL], FONT_SYSTEM, strbuf, 0, 0, TEXT_SPEED_NO_TRANSFER, TEXT_COLOR(15, 14, 0), NULL);
    Strbuf_Free(strbuf);
    ...
    StringTemplate_Format(battleParty->stringTemplate, formattedStrbuf, strbuf);
    Text_AddPrinterWithParamsAndColor(&battleParty->windows[BATTLE_PARTY_POKEMON_SUMMARY_SCREEN_WINDOW_LEVEL + windowIndexOffset], FONT_SYSTEM, formattedStrbuf, 0, 0, TEXT_SPEED_NO_TRANSFER, TEXT_COLOR(15, 14, 0), NULL);
    Strbuf_Free(strbuf);
    Strbuf_Free(formattedStrbuf);

    strbuf = MessageLoader_GetNewStrbuf(battleParty->messageLoader, BattleParty_Text_PokemonExpToNextLevelLabel);

    Text_AddPrinterWithParamsAndColor(&battleParty->windows[BATTLE_PARTY_POKEMON_SUMMARY_SCREEN_WINDOW_EXP_TO_NEXT_LVL_LABEL], FONT_SYSTEM, strbuf, 0, 0, TEXT_SPEED_NO_TRANSFER, TEXT_COLOR(15, 14, 0), NULL);
    Strbuf_Free(strbuf);
    ...
    xOffset = Window_GetWidth(&battleParty->windows[BATTLE_PARTY_POKEMON_SUMMARY_SCREEN_WINDOW_EXP_TO_NEXT_LVL + windowIndexOffset]) * 8 - Font_CalcStrbufWidth(FONT_SYSTEM, formattedStrbuf, 0);

    Text_AddPrinterWithParamsAndColor(&battleParty->windows[BATTLE_PARTY_POKEMON_SUMMARY_SCREEN_WINDOW_EXP_TO_NEXT_LVL + windowIndexOffset], FONT_SYSTEM, formattedStrbuf, xOffset, 0, TEXT_SPEED_NO_TRANSFER, TEXT_COLOR(1, 2, 0), NULL);
    Strbuf_Free(strbuf);
    Strbuf_Free(formattedStrbuf);
    Window_ScheduleCopyToVRAM(&battleParty->windows[BATTLE_PARTY_POKEMON_SUMMARY_SCREEN_WINDOW_LEVEL_LABEL]);
    Window_ScheduleCopyToVRAM(&battleParty->windows[BATTLE_PARTY_POKEMON_SUMMARY_SCREEN_WINDOW_LEVEL + windowIndexOffset]);
    Window_ScheduleCopyToVRAM(&battleParty->windows[BATTLE_PARTY_POKEMON_SUMMARY_SCREEN_WINDOW_EXP_TO_NEXT_LVL_LABEL]);
    Window_ScheduleCopyToVRAM(&battleParty->windows[BATTLE_PARTY_POKEMON_SUMMARY_SCREEN_WINDOW_EXP_TO_NEXT_LVL + windowIndexOffset]);
}
...
static void PrintPokemonAttackStat(BattleParty *battleParty, u32 partyIndex)
{
    ...
    Text_AddPrinterWithParamsAndColor(&battleParty->windows[BATTLE_PARTY_POKEMON_SUMMARY_SCREEN_WINDOW_ATTACK_LABEL], FONT_SYSTEM, strbuf, 0, 0, TEXT_SPEED_NO_TRANSFER, TEXT_COLOR(15, 14, 0), NULL);
    Strbuf_Free(strbuf);
    ...
    xOffset = Window_GetWidth(&battleParty->windows[BATTLE_PARTY_POKEMON_SUMMARY_SCREEN_WINDOW_ATTACK + windowIndexOffset]) * 8 - stringWidth;

    Text_AddPrinterWithParamsAndColor(&battleParty->windows[BATTLE_PARTY_POKEMON_SUMMARY_SCREEN_WINDOW_ATTACK + windowIndexOffset], FONT_SYSTEM, formattedStrbuf, xOffset, 0, TEXT_SPEED_NO_TRANSFER, TEXT_COLOR(1, 2, 0), NULL);
    Strbuf_Free(strbuf);
    Strbuf_Free(formattedStrbuf);
    Window_ScheduleCopyToVRAM(&battleParty->windows[BATTLE_PARTY_POKEMON_SUMMARY_SCREEN_WINDOW_ATTACK_LABEL]);
    Window_ScheduleCopyToVRAM(&battleParty->windows[BATTLE_PARTY_POKEMON_SUMMARY_SCREEN_WINDOW_ATTACK + windowIndexOffset]);
}
...
static void PrintPokemonDefenseStat(BattleParty *battleParty, u32 partyIndex)
{
    ...
    Text_AddPrinterWithParamsAndColor(&battleParty->windows[BATTLE_PARTY_POKEMON_SUMMARY_SCREEN_WINDOW_DEFENSE_LABEL], FONT_SYSTEM, strbuf, 0, 0, TEXT_SPEED_NO_TRANSFER, TEXT_COLOR(15, 14, 0), NULL);
    Strbuf_Free(strbuf);
    ...
    stringWidth = Font_CalcStrbufWidth(FONT_SYSTEM, formattedStrbuf, 0);
    xOffset = Window_GetWidth(&battleParty->windows[BATTLE_PARTY_POKEMON_SUMMARY_SCREEN_WINDOW_DEFENSE + windowIndexOffset]) * 8 - stringWidth;

    Text_AddPrinterWithParamsAndColor(&battleParty->windows[BATTLE_PARTY_POKEMON_SUMMARY_SCREEN_WINDOW_DEFENSE + windowIndexOffset], FONT_SYSTEM, formattedStrbuf, xOffset, 0, TEXT_SPEED_NO_TRANSFER, TEXT_COLOR(1, 2, 0), NULL);
    Strbuf_Free(strbuf);
    Strbuf_Free(formattedStrbuf);
    Window_ScheduleCopyToVRAM(&battleParty->windows[BATTLE_PARTY_POKEMON_SUMMARY_SCREEN_WINDOW_DEFENSE_LABEL]);
    Window_ScheduleCopyToVRAM(&battleParty->windows[BATTLE_PARTY_POKEMON_SUMMARY_SCREEN_WINDOW_DEFENSE + windowIndexOffset]);
}
...
static void PrintPokemonSpeedStat(BattleParty *battleParty, u32 partyIndex)
{
    ...
    Text_AddPrinterWithParamsAndColor(&battleParty->windows[BATTLE_PARTY_POKEMON_SUMMARY_SCREEN_WINDOW_SPEED_LABEL], FONT_SYSTEM, strbuf, 0, 0, TEXT_SPEED_NO_TRANSFER, TEXT_COLOR(15, 14, 0), NULL);
    Strbuf_Free(strbuf);
    ...
    stringWidth = Font_CalcStrbufWidth(FONT_SYSTEM, formattedStrbuf, 0);
    xOffset = Window_GetWidth(&battleParty->windows[BATTLE_PARTY_POKEMON_SUMMARY_SCREEN_WINDOW_SPEED + windowIndexOffset]) * 8 - stringWidth;

    Text_AddPrinterWithParamsAndColor(&battleParty->windows[BATTLE_PARTY_POKEMON_SUMMARY_SCREEN_WINDOW_SPEED + windowIndexOffset], FONT_SYSTEM, formattedStrbuf, xOffset, 0, TEXT_SPEED_NO_TRANSFER, TEXT_COLOR(1, 2, 0), NULL);
    Strbuf_Free(strbuf);
    Strbuf_Free(formattedStrbuf);
    Window_ScheduleCopyToVRAM(&battleParty->windows[BATTLE_PARTY_POKEMON_SUMMARY_SCREEN_WINDOW_SPEED_LABEL]);
    Window_ScheduleCopyToVRAM(&battleParty->windows[BATTLE_PARTY_POKEMON_SUMMARY_SCREEN_WINDOW_SPEED + windowIndexOffset]);
}
...
static void PrintPokemonSpAtkStat(BattleParty *battleParty, u32 partyIndex)
{
    ...
    Text_AddPrinterWithParamsAndColor(&battleParty->windows[BATTLE_PARTY_POKEMON_SUMMARY_SCREEN_WINDOW_SP_ATK_LABEL], FONT_SYSTEM, strbuf, 0, 0, TEXT_SPEED_NO_TRANSFER, TEXT_COLOR(15, 14, 0), NULL);
    Strbuf_Free(strbuf);
    ...
    stringWidth = Font_CalcStrbufWidth(FONT_SYSTEM, formattedStrbuf, 0);
    xOffset = Window_GetWidth(&battleParty->windows[BATTLE_PARTY_POKEMON_SUMMARY_SCREEN_WINDOW_SP_ATK + windowIndexOffset]) * 8 - stringWidth;

    Text_AddPrinterWithParamsAndColor(&battleParty->windows[BATTLE_PARTY_POKEMON_SUMMARY_SCREEN_WINDOW_SP_ATK + windowIndexOffset], FONT_SYSTEM, formattedStrbuf, xOffset, 0, TEXT_SPEED_NO_TRANSFER, TEXT_COLOR(1, 2, 0), NULL);
    Strbuf_Free(strbuf);
    Strbuf_Free(formattedStrbuf);
    Window_ScheduleCopyToVRAM(&battleParty->windows[BATTLE_PARTY_POKEMON_SUMMARY_SCREEN_WINDOW_SP_ATK_LABEL]);
    Window_ScheduleCopyToVRAM(&battleParty->windows[BATTLE_PARTY_POKEMON_SUMMARY_SCREEN_WINDOW_SP_ATK + windowIndexOffset]);
}
...
static void PrintPokemonSpDefStat(BattleParty *battleParty, u32 partyIndex)
{
    ...
    Text_AddPrinterWithParamsAndColor(&battleParty->windows[BATTLE_PARTY_POKEMON_SUMMARY_SCREEN_WINDOW_SP_DEF_LABEL], FONT_SYSTEM, strbuf, 0, 0, TEXT_SPEED_NO_TRANSFER, TEXT_COLOR(15, 14, 0), NULL);
    Strbuf_Free(strbuf);
    ...
    stringWidth = Font_CalcStrbufWidth(FONT_SYSTEM, formattedStrbuf, 0);
    xOffset = Window_GetWidth(&battleParty->windows[BATTLE_PARTY_POKEMON_SUMMARY_SCREEN_WINDOW_SP_DEF + windowIndexOffset]) * 8 - stringWidth;

    Text_AddPrinterWithParamsAndColor(&battleParty->windows[BATTLE_PARTY_POKEMON_SUMMARY_SCREEN_WINDOW_SP_DEF + windowIndexOffset], FONT_SYSTEM, formattedStrbuf, xOffset, 0, TEXT_SPEED_NO_TRANSFER, TEXT_COLOR(1, 2, 0), NULL);
    Strbuf_Free(strbuf);
    Strbuf_Free(formattedStrbuf);
    Window_ScheduleCopyToVRAM(&battleParty->windows[BATTLE_PARTY_POKEMON_SUMMARY_SCREEN_WINDOW_SP_DEF_LABEL]);
    Window_ScheduleCopyToVRAM(&battleParty->windows[BATTLE_PARTY_POKEMON_SUMMARY_SCREEN_WINDOW_SP_DEF + windowIndexOffset]);
}
...
static void PrintPokemonHPStat(BattleParty *battleParty, u32 partyIndex)
{
    ...
    Text_AddPrinterWithParamsAndColor(&battleParty->windows[BATTLE_PARTY_POKEMON_SUMMARY_SCREEN_WINDOW_HP_LABEL], FONT_SYSTEM, strbuf, 0, 0, TEXT_SPEED_NO_TRANSFER, TEXT_COLOR(15, 14, 0), NULL);
    Strbuf_Free(strbuf);
    ...
    xOffset = (Window_GetWidth(&battleParty->windows[BATTLE_PARTY_POKEMON_SUMMARY_SCREEN_WINDOW_HP]) * 8 - stringWidth) / 2;

    Text_AddPrinterWithParamsAndColor(&battleParty->windows[BATTLE_PARTY_POKEMON_SUMMARY_SCREEN_WINDOW_HP + windowIndexOffset], FONT_SYSTEM, strbuf, xOffset, 0, TEXT_SPEED_NO_TRANSFER, TEXT_COLOR(1, 2, 0), NULL);
    Strbuf_Free(strbuf);
    ...
    Text_AddPrinterWithParamsAndColor(&battleParty->windows[BATTLE_PARTY_POKEMON_SUMMARY_SCREEN_WINDOW_HP + windowIndexOffset], FONT_SYSTEM, formattedStrbuf, xOffset - formattedStringWidth, 0, TEXT_SPEED_NO_TRANSFER, TEXT_COLOR(1, 2, 0), NULL);
    Strbuf_Free(strbuf);
    Strbuf_Free(formattedStrbuf);
    ...
    StringTemplate_Format(battleParty->stringTemplate, formattedStrbuf, strbuf);
    Text_AddPrinterWithParamsAndColor(&battleParty->windows[BATTLE_PARTY_POKEMON_SUMMARY_SCREEN_WINDOW_HP + windowIndexOffset], FONT_SYSTEM, formattedStrbuf, xOffset + stringWidth, 0, TEXT_SPEED_NO_TRANSFER, TEXT_COLOR(1, 2, 0), NULL);
    Strbuf_Free(strbuf);
    Strbuf_Free(formattedStrbuf);
    Window_ScheduleCopyToVRAM(&battleParty->windows[BATTLE_PARTY_POKEMON_SUMMARY_SCREEN_WINDOW_HP_LABEL]);
    Window_ScheduleCopyToVRAM(&battleParty->windows[BATTLE_PARTY_POKEMON_SUMMARY_SCREEN_WINDOW_HP + windowIndexOffset]);
}

static void PrintPokemonAbilityDescription(BattleParty *battleParty, u32 partyIndex)
{
    BattlePartyPokemon *pokemon = &battleParty->partyPokemon[partyIndex];
    u32 windowIndexOffset = windowIndexOffset = BATTLE_PARTY_POKEMON_SUMMARY_SCREEN_WINDOW_ALT_POKEMON_NAME * battleParty->useAltSummaryWindows;
    ...
    Text_AddPrinterWithParamsAndColor(&battleParty->windows[BATTLE_PARTY_POKEMON_SUMMARY_SCREEN_WINDOW_ABILITY_DESC + windowIndexOffset], FONT_SYSTEM, strbuf, 0, 0, TEXT_SPEED_NO_TRANSFER, TEXT_COLOR(1, 2, 0), NULL);
    Strbuf_Free(strbuf);
    MessageLoader_Free(messageLoader);
    Window_ScheduleCopyToVRAM(&battleParty->windows[BATTLE_PARTY_POKEMON_SUMMARY_SCREEN_WINDOW_ABILITY_DESC + windowIndexOffset]);
}
...
static void RenderSelectPokemonScreen(BattleParty *battleParty)
{
    Window_FillTilemap(&battleParty->windows[BATTLE_PARTY_SELECT_POKEMON_SCREEN_WINDOW_POKEMON_NAME], 0);
    Window_FillTilemap(&battleParty->windows[BATTLE_PARTY_SELECT_POKEMON_SCREEN_WINDOW_SHIFT], 0);
    Window_FillTilemap(&battleParty->windows[BATTLE_PARTY_SELECT_POKEMON_SCREEN_WINDOW_SUMMARY], 0);
    Window_FillTilemap(&battleParty->windows[BATTLE_PARTY_SELECT_POKEMON_SCREEN_WINDOW_CHECK_MOVES], 0);

    PrintSelectedPokemonName(battleParty, battleParty->context->selectedPartyIndex);

    PrintButtonText(battleParty, BATTLE_PARTY_SELECT_POKEMON_SCREEN_WINDOW_SHIFT, BattleParty_Text_ShiftButton);

    if (battleParty->partyPokemon[battleParty->context->selectedPartyIndex].isEgg == FALSE) {
        PrintButtonText(battleParty, BATTLE_PARTY_SELECT_POKEMON_SCREEN_WINDOW_SUMMARY, BattleParty_Text_SummaryButton);
        PrintButtonText(battleParty, BATTLE_PARTY_SELECT_POKEMON_SCREEN_WINDOW_CHECK_MOVES, BattleParty_Text_CheckMovesButton);
    } else {
        Window_ScheduleCopyToVRAM(&battleParty->windows[BATTLE_PARTY_SELECT_POKEMON_SCREEN_WINDOW_SUMMARY]);
        Window_ScheduleCopyToVRAM(&battleParty->windows[BATTLE_PARTY_SELECT_POKEMON_SCREEN_WINDOW_CHECK_MOVES]);
    }
}

static void PrintSelectedPokemonName(BattleParty *battleParty, u32 partyIndex)
{
    Window *window = &battleParty->windows[BATTLE_PARTY_SELECT_POKEMON_SCREEN_WINDOW_POKEMON_NAME];
    BattlePartyPokemon *pokemon = &battleParty->partyPokemon[partyIndex];
...
static void RenderPokemonMovesScreen(BattleParty *battleParty)
{
    u16 windowIndexOffset = BATTLE_PARTY_POKEMON_MOVES_SCREEN_WINDOW_ALT_POKEMON_NAME * battleParty->useAltSummaryWindows;

    Window_FillTilemap(&battleParty->windows[BATTLE_PARTY_POKEMON_MOVES_SCREEN_WINDOW_POKEMON_NAME + windowIndexOffset], 0);
    Window_FillTilemap(&battleParty->windows[BATTLE_PARTY_POKEMON_MOVES_SCREEN_WINDOW_MOVE_1 + windowIndexOffset], 0);
    Window_FillTilemap(&battleParty->windows[BATTLE_PARTY_POKEMON_MOVES_SCREEN_WINDOW_MOVE_2 + windowIndexOffset], 0);
    Window_FillTilemap(&battleParty->windows[BATTLE_PARTY_POKEMON_MOVES_SCREEN_WINDOW_MOVE_3 + windowIndexOffset], 0);
    Window_FillTilemap(&battleParty->windows[BATTLE_PARTY_POKEMON_MOVES_SCREEN_WINDOW_MOVE_4 + windowIndexOffset], 0);
    Window_FillTilemap(&battleParty->windows[BATTLE_PARTY_POKEMON_MOVES_SCREEN_WINDOW_SUMMARY], 0);
    ...
    for (u16 i = 0; i < LEARNED_MOVES_MAX; i++) {
        ...
        PrintMoveName(battleParty, move->move, BATTLE_PARTY_POKEMON_MOVES_SCREEN_WINDOW_MOVE_1 + windowIndexOffset + i, sMoveNameTextIDs[i], FONT_SUBSCREEN, 8, TEXT_COLOR(7, 8, 9));
        PrintMovesScreenMovePPStats(battleParty, move, BATTLE_PARTY_POKEMON_MOVES_SCREEN_WINDOW_MOVE_1 + windowIndexOffset + i);
    }

    PrintButtonText(battleParty, BATTLE_PARTY_POKEMON_MOVES_SCREEN_WINDOW_SUMMARY, BattleParty_Text_SummaryButton);

    Window_ScheduleCopyToVRAM(&battleParty->windows[BATTLE_PARTY_POKEMON_MOVES_SCREEN_WINDOW_MOVE_1 + windowIndexOffset]);
    Window_ScheduleCopyToVRAM(&battleParty->windows[BATTLE_PARTY_POKEMON_MOVES_SCREEN_WINDOW_MOVE_2 + windowIndexOffset]);
    Window_ScheduleCopyToVRAM(&battleParty->windows[BATTLE_PARTY_POKEMON_MOVES_SCREEN_WINDOW_MOVE_3 + windowIndexOffset]);
    Window_ScheduleCopyToVRAM(&battleParty->windows[BATTLE_PARTY_POKEMON_MOVES_SCREEN_WINDOW_MOVE_4 + windowIndexOffset]);
...
static void RenderPokemonSummaryScreen(BattleParty *battleParty)
{
    u32 windowIndexOffset = BATTLE_PARTY_POKEMON_SUMMARY_SCREEN_WINDOW_ALT_POKEMON_NAME * battleParty->useAltSummaryWindows;

    Window_FillTilemap(&battleParty->windows[BATTLE_PARTY_POKEMON_SUMMARY_SCREEN_WINDOW_LEVEL_LABEL], 0);
    Window_FillTilemap(&battleParty->windows[BATTLE_PARTY_POKEMON_SUMMARY_SCREEN_WINDOW_EXP_TO_NEXT_LVL_LABEL], 0);
    Window_FillTilemap(&battleParty->windows[BATTLE_PARTY_POKEMON_SUMMARY_SCREEN_WINDOW_ATTACK_LABEL], 0);
    Window_FillTilemap(&battleParty->windows[BATTLE_PARTY_POKEMON_SUMMARY_SCREEN_WINDOW_DEFENSE_LABEL], 0);
    Window_FillTilemap(&battleParty->windows[BATTLE_PARTY_POKEMON_SUMMARY_SCREEN_WINDOW_SPEED_LABEL], 0);
    Window_FillTilemap(&battleParty->windows[BATTLE_PARTY_POKEMON_SUMMARY_SCREEN_WINDOW_SP_ATK_LABEL], 0);
    Window_FillTilemap(&battleParty->windows[BATTLE_PARTY_POKEMON_SUMMARY_SCREEN_WINDOW_SP_DEF_LABEL], 0);
    Window_FillTilemap(&battleParty->windows[BATTLE_PARTY_POKEMON_SUMMARY_SCREEN_WINDOW_HP_LABEL], 0);
    Window_FillTilemap(&battleParty->windows[BATTLE_PARTY_POKEMON_SUMMARY_SCREEN_WINDOW_CHECK_MOVES], 0);

    Window_FillTilemap(&battleParty->windows[BATTLE_PARTY_POKEMON_SUMMARY_SCREEN_WINDOW_POKEMON_NAME + windowIndexOffset], 0);
    Window_FillTilemap(&battleParty->windows[BATTLE_PARTY_POKEMON_SUMMARY_SCREEN_WINDOW_HEALTH_BAR + windowIndexOffset], 0);
    Window_FillTilemap(&battleParty->windows[BATTLE_PARTY_POKEMON_SUMMARY_SCREEN_WINDOW_LEVEL + windowIndexOffset], 0);
    Window_FillTilemap(&battleParty->windows[BATTLE_PARTY_POKEMON_SUMMARY_SCREEN_WINDOW_EXP_TO_NEXT_LVL + windowIndexOffset], 0);
    Window_FillTilemap(&battleParty->windows[BATTLE_PARTY_POKEMON_SUMMARY_SCREEN_WINDOW_ATTACK + windowIndexOffset], 0);
    Window_FillTilemap(&battleParty->windows[BATTLE_PARTY_POKEMON_SUMMARY_SCREEN_WINDOW_DEFENSE + windowIndexOffset], 0);
    Window_FillTilemap(&battleParty->windows[BATTLE_PARTY_POKEMON_SUMMARY_SCREEN_WINDOW_SPEED + windowIndexOffset], 0);
    Window_FillTilemap(&battleParty->windows[BATTLE_PARTY_POKEMON_SUMMARY_SCREEN_WINDOW_SP_ATK + windowIndexOffset], 0);
    Window_FillTilemap(&battleParty->windows[BATTLE_PARTY_POKEMON_SUMMARY_SCREEN_WINDOW_SP_DEF + windowIndexOffset], 0);
    Window_FillTilemap(&battleParty->windows[BATTLE_PARTY_POKEMON_SUMMARY_SCREEN_WINDOW_HP + windowIndexOffset], 0);
    Window_FillTilemap(&battleParty->windows[BATTLE_PARTY_POKEMON_SUMMARY_SCREEN_WINDOW_ABILITY_NAME + windowIndexOffset], 0);
    Window_FillTilemap(&battleParty->windows[BATTLE_PARTY_POKEMON_SUMMARY_SCREEN_WINDOW_ABILITY_DESC + windowIndexOffset], 0);
    Window_FillTilemap(&battleParty->windows[BATTLE_PARTY_POKEMON_SUMMARY_SCREEN_WINDOW_HELD_ITEM + windowIndexOffset], 0);

    PrintPokemonNameHeader(battleParty, BATTLE_PARTY_POKEMON_SUMMARY_SCREEN_WINDOW_POKEMON_NAME + windowIndexOffset, FONT_SYSTEM, battleParty->context->selectedPartyIndex, 0, 0);
    PrintPokemonHPStat(battleParty, battleParty->context->selectedPartyIndex);
    DrawPokemonHealthBar(battleParty, BATTLE_PARTY_POKEMON_SUMMARY_SCREEN_WINDOW_HEALTH_BAR + windowIndexOffset, battleParty->context->selectedPartyIndex, 0, 0);
    ...
    PrintPokemonAbilityName(battleParty, BATTLE_PARTY_POKEMON_SUMMARY_SCREEN_WINDOW_ABILITY_NAME + windowIndexOffset, battleParty->context->selectedPartyIndex);
    PrintPokemonHeldItem(battleParty, BATTLE_PARTY_POKEMON_SUMMARY_SCREEN_WINDOW_HELD_ITEM + windowIndexOffset, battleParty->context->selectedPartyIndex);
    PrintPokemonAbilityDescription(battleParty, battleParty->context->selectedPartyIndex);
    PrintButtonText(battleParty, BATTLE_PARTY_POKEMON_SUMMARY_SCREEN_WINDOW_CHECK_MOVES, BattleParty_Text_CheckMovesButton);
...
static void RenderMoveSummaryScreen(BattleParty *battleParty)
{
    u32 windowIndexOffset = BATTLE_PARTY_MOVE_SUMMARY_SCREEN_WINDOW_ALT_MOVE_NAME * battleParty->useAltSummaryWindows;
    BattlePartyPokemonMove *move;

    Window_FillTilemap(&battleParty->windows[BATTLE_PARTY_MOVE_SUMMARY_SCREEN_WINDOW_POKEMON_NAME], 0);
    Window_FillTilemap(&battleParty->windows[BATTLE_PARTY_MOVE_SUMMARY_SCREEN_WINDOW_PP_LABEL], 0);
    Window_FillTilemap(&battleParty->windows[BATTLE_PARTY_MOVE_SUMMARY_SCREEN_WINDOW_ACCURACY_LABEL], 0);
    Window_FillTilemap(&battleParty->windows[BATTLE_PARTY_MOVE_SUMMARY_SCREEN_WINDOW_POWER_LABEL], 0);
    Window_FillTilemap(&battleParty->windows[BATTLE_PARTY_MOVE_SUMMARY_SCREEN_WINDOW_CATEGORY_LABEL], 0);
    Window_FillTilemap(&battleParty->windows[BATTLE_PARTY_MOVE_SUMMARY_SCREEN_WINDOW_PP + windowIndexOffset], 0);
    Window_FillTilemap(&battleParty->windows[BATTLE_PARTY_MOVE_SUMMARY_SCREEN_WINDOW_MOVE_NAME + windowIndexOffset], 0);
    Window_FillTilemap(&battleParty->windows[BATTLE_PARTY_MOVE_SUMMARY_SCREEN_WINDOW_ACCURACY + windowIndexOffset], 0);
    Window_FillTilemap(&battleParty->windows[BATTLE_PARTY_MOVE_SUMMARY_SCREEN_WINDOW_POWER + windowIndexOffset], 0);
    Window_FillTilemap(&battleParty->windows[BATTLE_PARTY_MOVE_SUMMARY_SCREEN_WINDOW_CATEGORY + windowIndexOffset], 0);
    Window_FillTilemap(&battleParty->windows[BATTLE_PARTY_MOVE_SUMMARY_SCREEN_WINDOW_DESCRIPTION + windowIndexOffset], 0);

    move = &battleParty->partyPokemon[battleParty->context->selectedPartyIndex].moves[battleParty->context->selectedMoveSlot];

    PrintPokemonNameHeader(battleParty, BATTLE_PARTY_MOVE_SUMMARY_SCREEN_WINDOW_POKEMON_NAME, FONT_SYSTEM, battleParty->context->selectedPartyIndex, 0, 0);
    PrintPPLabel(battleParty, BATTLE_PARTY_MOVE_SUMMARY_SCREEN_WINDOW_PP_LABEL, 0, 0);
    PrintMoveName(battleParty, move->move, BATTLE_PARTY_MOVE_SUMMARY_SCREEN_WINDOW_MOVE_NAME + windowIndexOffset, sMoveNameTextIDs[battleParty->context->selectedMoveSlot], FONT_SYSTEM, 0, TEXT_COLOR(15, 14, 0));
    PrintMoveAccuracyLabel(battleParty, BATTLE_PARTY_MOVE_SUMMARY_SCREEN_WINDOW_ACCURACY_LABEL);
    PrintMoveAccuracyValue(battleParty, BATTLE_PARTY_MOVE_SUMMARY_SCREEN_WINDOW_ACCURACY + windowIndexOffset, move->accuracy);
    PrintMovePowerLabel(battleParty, BATTLE_PARTY_MOVE_SUMMARY_SCREEN_WINDOW_POWER_LABEL);
    PrintMovePowerValue(battleParty, BATTLE_PARTY_MOVE_SUMMARY_SCREEN_WINDOW_POWER + windowIndexOffset, move->power);
    PrintMoveDescription(battleParty, BATTLE_PARTY_MOVE_SUMMARY_SCREEN_WINDOW_DESCRIPTION + windowIndexOffset, move->move);
    PrintMoveCategoryLabel(battleParty, BATTLE_PARTY_MOVE_SUMMARY_SCREEN_WINDOW_CATEGORY_LABEL);
    PrintMoveCategory(battleParty, BATTLE_PARTY_MOVE_SUMMARY_SCREEN_WINDOW_CATEGORY + windowIndexOffset, move->class);
    PrintSummaryScreenMovePPStats(battleParty, BATTLE_PARTY_MOVE_SUMMARY_SCREEN_WINDOW_PP + windowIndexOffset, move->currentPP, move->maxPP);
...
static void RenderLearnMoveScreen(BattleParty *battleParty)
{
    ...
    Window_FillTilemap(&battleParty->windows[BATTLE_PARTY_LEARN_MOVE_SCREEN_WINDOW_POKEMON_NAME], 0);
    Window_FillTilemap(&battleParty->windows[BATTLE_PARTY_LEARN_MOVE_SCREEN_WINDOW_MOVE_1], 0);
    Window_FillTilemap(&battleParty->windows[BATTLE_PARTY_LEARN_MOVE_SCREEN_WINDOW_MOVE_2], 0);
    Window_FillTilemap(&battleParty->windows[BATTLE_PARTY_LEARN_MOVE_SCREEN_WINDOW_MOVE_3], 0);
    Window_FillTilemap(&battleParty->windows[BATTLE_PARTY_LEARN_MOVE_SCREEN_WINDOW_MOVE_4], 0);
    Window_FillTilemap(&battleParty->windows[BATTLE_PARTY_LEARN_MOVE_SCREEN_WINDOW_MOVE_TO_LEARN], 0);

    PrintPokemonNameHeader(battleParty, BATTLE_PARTY_LEARN_MOVE_SCREEN_WINDOW_POKEMON_NAME, FONT_SYSTEM, battleParty->context->selectedPartyIndex, 0, 0);
    ...
    for (u32 i = 0; i < LEARNED_MOVES_MAX; i++) {
        ...
        PrintMoveName(battleParty, move->move, BATTLE_PARTY_LEARN_MOVE_SCREEN_WINDOW_MOVE_1 + i, sMoveNameTextIDs[i], FONT_SUBSCREEN, 8, TEXT_COLOR(7, 8, 9));
        PrintMovesScreenMovePPStats(battleParty, move, BATTLE_PARTY_LEARN_MOVE_SCREEN_WINDOW_MOVE_1 + i);
    }

    PrintMoveName(battleParty, battleParty->context->moveToLearn, BATTLE_PARTY_LEARN_MOVE_SCREEN_WINDOW_MOVE_TO_LEARN, sMoveNameTextIDs[MOVE_TO_LEARN_SLOT], FONT_SUBSCREEN, 8, TEXT_COLOR(7, 8, 9));
    ...
    PrintMovesScreenMovePPStats(battleParty, &moveToLearn, BATTLE_PARTY_LEARN_MOVE_SCREEN_WINDOW_MOVE_TO_LEARN);

    Window_ScheduleCopyToVRAM(&battleParty->windows[BATTLE_PARTY_LEARN_MOVE_SCREEN_WINDOW_MOVE_1]);
    Window_ScheduleCopyToVRAM(&battleParty->windows[BATTLE_PARTY_LEARN_MOVE_SCREEN_WINDOW_MOVE_2]);
    Window_ScheduleCopyToVRAM(&battleParty->windows[BATTLE_PARTY_LEARN_MOVE_SCREEN_WINDOW_MOVE_3]);
    Window_ScheduleCopyToVRAM(&battleParty->windows[BATTLE_PARTY_LEARN_MOVE_SCREEN_WINDOW_MOVE_4]);
    Window_ScheduleCopyToVRAM(&battleParty->windows[BATTLE_PARTY_LEARN_MOVE_SCREEN_WINDOW_MOVE_TO_LEARN]);
}

static void RenderConfirmLearnMoveScreen(BattleParty *battleParty)
{
    Window_FillTilemap(&battleParty->windows[BATTLE_PARTY_CONFIRM_LEARN_MOVE_SCREEN_WINDOW_POKEMON_NAME], 0);
    Window_FillTilemap(&battleParty->windows[BATTLE_PARTY_CONFIRM_LEARN_MOVE_SCREEN_WINDOW_PP_LABEL], 0);
    Window_FillTilemap(&battleParty->windows[BATTLE_PARTY_CONFIRM_LEARN_MOVE_SCREEN_WINDOW_PP], 0);
    Window_FillTilemap(&battleParty->windows[BATTLE_PARTY_CONFIRM_LEARN_MOVE_SCREEN_WINDOW_MOVE_NAME], 0);
    Window_FillTilemap(&battleParty->windows[BATTLE_PARTY_CONFIRM_LEARN_MOVE_SCREEN_WINDOW_ACCURACY_LABEL], 0);
    Window_FillTilemap(&battleParty->windows[BATTLE_PARTY_CONFIRM_LEARN_MOVE_SCREEN_WINDOW_ACCURACY], 0);
    Window_FillTilemap(&battleParty->windows[BATTLE_PARTY_CONFIRM_LEARN_MOVE_SCREEN_WINDOW_POWER_LABEL], 0);
    Window_FillTilemap(&battleParty->windows[BATTLE_PARTY_CONFIRM_LEARN_MOVE_SCREEN_WINDOW_POWER], 0);
    Window_FillTilemap(&battleParty->windows[BATTLE_PARTY_CONFIRM_LEARN_MOVE_SCREEN_WINDOW_CATEGORY_LABEL], 0);
    Window_FillTilemap(&battleParty->windows[BATTLE_PARTY_CONFIRM_LEARN_MOVE_SCREEN_WINDOW_CATEGORY], 0);
    Window_FillTilemap(&battleParty->windows[BATTLE_PARTY_CONFIRM_LEARN_MOVE_SCREEN_WINDOW_DESCRIPTION], 0);
    Window_FillTilemap(&battleParty->windows[BATTLE_PARTY_CONFIRM_LEARN_MOVE_SCREEN_WINDOW_CONFIRM], 0);

    PrintPokemonNameHeader(battleParty, BATTLE_PARTY_CONFIRM_LEARN_MOVE_SCREEN_WINDOW_POKEMON_NAME, FONT_SYSTEM, battleParty->context->selectedPartyIndex, 0, 0);
    PrintPPLabel(battleParty, BATTLE_PARTY_CONFIRM_LEARN_MOVE_SCREEN_WINDOW_PP_LABEL, 0, 0);
    PrintMoveAccuracyLabel(battleParty, BATTLE_PARTY_CONFIRM_LEARN_MOVE_SCREEN_WINDOW_ACCURACY_LABEL);
    PrintMovePowerLabel(battleParty, BATTLE_PARTY_CONFIRM_LEARN_MOVE_SCREEN_WINDOW_POWER_LABEL);
    PrintMoveCategoryLabel(battleParty, BATTLE_PARTY_CONFIRM_LEARN_MOVE_SCREEN_WINDOW_CATEGORY_LABEL);
    ...
    if (battleParty->context->selectedMoveSlot < MOVE_TO_LEARN_SLOT) {
        ...
        PrintMoveName(battleParty, move->move, BATTLE_PARTY_CONFIRM_LEARN_MOVE_SCREEN_WINDOW_MOVE_NAME, sMoveNameTextIDs[battleParty->context->selectedMoveSlot], FONT_SYSTEM, 0, TEXT_COLOR(15, 14, 0));
        PrintMoveAccuracyValue(battleParty, BATTLE_PARTY_CONFIRM_LEARN_MOVE_SCREEN_WINDOW_ACCURACY, move->accuracy);
        PrintMovePowerValue(battleParty, BATTLE_PARTY_CONFIRM_LEARN_MOVE_SCREEN_WINDOW_POWER, move->power);
        PrintMoveDescription(battleParty, BATTLE_PARTY_CONFIRM_LEARN_MOVE_SCREEN_WINDOW_DESCRIPTION, move->move);
        PrintMoveCategory(battleParty, BATTLE_PARTY_CONFIRM_LEARN_MOVE_SCREEN_WINDOW_CATEGORY, move->class);
        PrintSummaryScreenMovePPStats(battleParty, BATTLE_PARTY_CONFIRM_LEARN_MOVE_SCREEN_WINDOW_PP, move->currentPP, move->maxPP);
    } else {
        u32 pp = MoveTable_LoadParam(battleParty->context->moveToLearn, MOVEATTRIBUTE_PP);

        PrintMoveName(battleParty, battleParty->context->moveToLearn, BATTLE_PARTY_CONFIRM_LEARN_MOVE_SCREEN_WINDOW_MOVE_NAME, sMoveNameTextIDs[4], FONT_SYSTEM, 0, TEXT_COLOR(15, 14, 0));
        PrintMoveDescription(battleParty, BATTLE_PARTY_CONFIRM_LEARN_MOVE_SCREEN_WINDOW_DESCRIPTION, battleParty->context->moveToLearn);
        PrintMoveAccuracyValue(battleParty, BATTLE_PARTY_CONFIRM_LEARN_MOVE_SCREEN_WINDOW_ACCURACY, MoveTable_LoadParam(battleParty->context->moveToLearn, MOVEATTRIBUTE_ACCURACY));
        PrintMovePowerValue(battleParty, BATTLE_PARTY_CONFIRM_LEARN_MOVE_SCREEN_WINDOW_POWER, MoveTable_LoadParam(battleParty->context->moveToLearn, MOVEATTRIBUTE_POWER));
        PrintMoveCategory(battleParty, BATTLE_PARTY_CONFIRM_LEARN_MOVE_SCREEN_WINDOW_CATEGORY, MoveTable_LoadParam(battleParty->context->moveToLearn, MOVEATTRIBUTE_CLASS));
        PrintSummaryScreenMovePPStats(battleParty, BATTLE_PARTY_CONFIRM_LEARN_MOVE_SCREEN_WINDOW_PP, pp, pp);
    }

    PrintConfirmMoveButton(battleParty, BATTLE_PARTY_CONFIRM_LEARN_MOVE_SCREEN_WINDOW_CONFIRM);
}

static void RenderRestoreMovePPScreen(BattleParty *battleParty)
{
    Window_FillTilemap(&battleParty->windows[BATTLE_PARTY_RESTORE_MOVE_PP_SCREEN_WINDOW_POKEMON_NAME], 0);
    Window_FillTilemap(&battleParty->windows[BATTLE_PARTY_RESTORE_MOVE_PP_SCREEN_WINDOW_MOVE_1], 0);
    Window_FillTilemap(&battleParty->windows[BATTLE_PARTY_RESTORE_MOVE_PP_SCREEN_WINDOW_MOVE_2], 0);
    Window_FillTilemap(&battleParty->windows[BATTLE_PARTY_RESTORE_MOVE_PP_SCREEN_WINDOW_MOVE_3], 0);
    Window_FillTilemap(&battleParty->windows[BATTLE_PARTY_RESTORE_MOVE_PP_SCREEN_WINDOW_MOVE_4], 0);

    PrintPokemonNameHeader(battleParty, BATTLE_PARTY_RESTORE_MOVE_PP_SCREEN_WINDOW_POKEMON_NAME, FONT_SYSTEM, battleParty->context->selectedPartyIndex, 0, 0);
    ...
    for (u32 i = 0; i < LEARNED_MOVES_MAX; i++) {
        ...
        PrintMoveName(battleParty, move->move, BATTLE_PARTY_RESTORE_MOVE_PP_SCREEN_WINDOW_MOVE_1 + i, sMoveNameTextIDs[i], FONT_SUBSCREEN, 8, TEXT_COLOR(7, 8, 9));
        PrintMovesScreenMovePPStats(battleParty, move, BATTLE_PARTY_RESTORE_MOVE_PP_SCREEN_WINDOW_MOVE_1 + i);
    }
    ...
    Window_ScheduleCopyToVRAM(&battleParty->windows[BATTLE_PARTY_RESTORE_MOVE_PP_SCREEN_WINDOW_MOVE_1]);
    Window_ScheduleCopyToVRAM(&battleParty->windows[BATTLE_PARTY_RESTORE_MOVE_PP_SCREEN_WINDOW_MOVE_2]);
    Window_ScheduleCopyToVRAM(&battleParty->windows[BATTLE_PARTY_RESTORE_MOVE_PP_SCREEN_WINDOW_MOVE_3]);
    Window_ScheduleCopyToVRAM(&battleParty->windows[BATTLE_PARTY_RESTORE_MOVE_PP_SCREEN_WINDOW_MOVE_4]);
}
...
static void RenderConfirmLearnMoveContestStatsScreen(BattleParty *battleParty)
{
    ...
    Window_FillTilemap(&battleParty->windows[BATTLE_PARTY_CONFIRM_LEARN_MOVE_CONTEST_STATS_SCREEN_WINDOW_POKEMON_NAME], 0);
    Window_FillTilemap(&battleParty->windows[BATTLE_PARTY_CONFIRM_LEARN_MOVE_CONTEST_STATS_SCREEN_WINDOW_PP_LABEL], 0);
    Window_FillTilemap(&battleParty->windows[BATTLE_PARTY_CONFIRM_LEARN_MOVE_CONTEST_STATS_SCREEN_WINDOW_PP], 0);
    Window_FillTilemap(&battleParty->windows[BATTLE_PARTY_CONFIRM_LEARN_MOVE_CONTEST_STATS_SCREEN_WINDOW_MOVE_NAME], 0);
    Window_FillTilemap(&battleParty->windows[BATTLE_PARTY_CONFIRM_LEARN_MOVE_CONTEST_STATS_SCREEN_WINDOW_APPEAL_PTS], 0);
    Window_FillTilemap(&battleParty->windows[BATTLE_PARTY_CONFIRM_LEARN_MOVE_CONTEST_STATS_SCREEN_WINDOW_CONTEST_EFFECT], 0);
    Window_FillTilemap(&battleParty->windows[BATTLE_PARTY_CONFIRM_LEARN_MOVE_CONTEST_STATS_SCREEN_WINDOW_CONFIRM], 0);

    PrintPokemonNameHeader(battleParty, BATTLE_PARTY_CONFIRM_LEARN_MOVE_CONTEST_STATS_SCREEN_WINDOW_POKEMON_NAME, FONT_SYSTEM, battleParty->context->selectedPartyIndex, 0, 0);
    PrintPPLabel(battleParty, BATTLE_PARTY_CONFIRM_LEARN_MOVE_CONTEST_STATS_SCREEN_WINDOW_PP_LABEL, 0, 0);
    ...
    Text_AddPrinterWithParamsAndColor(&battleParty->windows[BATTLE_PARTY_CONFIRM_LEARN_MOVE_CONTEST_STATS_SCREEN_WINDOW_APPEAL_PTS], FONT_SYSTEM, strbuf, xOffset, 0, TEXT_SPEED_NO_TRANSFER, TEXT_COLOR(15, 14, 0), NULL);

    Strbuf_Free(strbuf);
    Window_ScheduleCopyToVRAM(&battleParty->windows[BATTLE_PARTY_CONFIRM_LEARN_MOVE_CONTEST_STATS_SCREEN_WINDOW_APPEAL_PTS]);

    if (battleParty->context->selectedMoveSlot < MOVE_TO_LEARN_SLOT) {
        BattlePartyPokemonMove *move = &battleParty->partyPokemon[battleParty->context->selectedPartyIndex].moves[battleParty->context->selectedMoveSlot];

        PrintMoveName(battleParty, move->move, BATTLE_PARTY_CONFIRM_LEARN_MOVE_CONTEST_STATS_SCREEN_WINDOW_MOVE_NAME, sMoveNameTextIDs[battleParty->context->selectedMoveSlot], FONT_SYSTEM, 0, TEXT_COLOR(15, 14, 0));
        PrintMoveContestEffect(battleParty, BATTLE_PARTY_CONFIRM_LEARN_MOVE_CONTEST_STATS_SCREEN_WINDOW_CONTEST_EFFECT, move->move);
        PrintSummaryScreenMovePPStats(battleParty, BATTLE_PARTY_CONFIRM_LEARN_MOVE_CONTEST_STATS_SCREEN_WINDOW_PP, move->currentPP, move->maxPP);
    } else {
        u32 pp = MoveTable_LoadParam(battleParty->context->moveToLearn, MOVEATTRIBUTE_PP);

        PrintMoveName(battleParty, battleParty->context->moveToLearn, BATTLE_PARTY_CONFIRM_LEARN_MOVE_CONTEST_STATS_SCREEN_WINDOW_MOVE_NAME, sMoveNameTextIDs[4], FONT_SYSTEM, 0, TEXT_COLOR(15, 14, 0));
        PrintMoveContestEffect(battleParty, BATTLE_PARTY_CONFIRM_LEARN_MOVE_CONTEST_STATS_SCREEN_WINDOW_CONTEST_EFFECT, battleParty->context->moveToLearn);
        PrintSummaryScreenMovePPStats(battleParty, BATTLE_PARTY_CONFIRM_LEARN_MOVE_CONTEST_STATS_SCREEN_WINDOW_PP, pp, pp);
    }

    PrintConfirmMoveButton(battleParty, BATTLE_PARTY_CONFIRM_LEARN_MOVE_CONTEST_STATS_SCREEN_WINDOW_CONFIRM);
}
