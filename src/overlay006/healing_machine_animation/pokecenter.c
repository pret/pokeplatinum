...
typedef struct PokecenterHealingAnimation {
    ...
    u8 pokeballTicks;
    u8 state;
    u8 pokeballLoadedPropIDs[PARTY_MAX_POKEMON];
} PokecenterHealingAnimation;

static BOOL FieldTask_PlayHealingAnimation_Pokecenter(FieldTask *param0);

static VecFx32 PokecenterHealingAnimation_PokeballOffsets[PARTY_MAX_POKEMON] = {
...
void FieldSystem_PlayHealingAnimation_Pokecenter(FieldSystem *fieldSystem, const u8 pokeballCount)
{
    ...
    if (mapPropFound) {
        PokecenterHealingAnimation *animation = Heap_AllocFromHeapAtEnd(HEAP_ID_FIELD, sizeof(PokecenterHealingAnimation));
