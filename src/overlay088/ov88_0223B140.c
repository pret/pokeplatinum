    ...
    case 0:
...

        {
            ...
            for (v0 = 0; v0 < Party_GetCurrentCount(param0->unk_08->unk_08); v0++) {
                Pokemon *mon = Party_GetPokemonBySlotIndex(param0->unk_08->unk_08, v0);

                species = GetMonData(mon, MON_DATA_SPECIES_EGG, NULL);

                if (species == SPECIES_SHAYMIN) {
                    form = GetMonData(mon, MON_DATA_FORM, NULL);

                    if (form != SHAYMIN_FORM_LAND) {
                        SetShayminForm(mon, SHAYMIN_FORM_LAND);
                    }
                }
            }
        }
...
static int ov88_0223C800 (int param0, Pokemon *param1, u8 * param2, UnkStruct_02008A90 * param3)
{
...

    {
        ...
        int species = GetMonData(param1, MON_DATA_SPECIES, NULL);

        sub_020136A4(param3->unk_00, param3->unk_02, 26, 0, 0, 10, 10, param2, v0, 0, 2, species);
    }
