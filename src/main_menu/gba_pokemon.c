...
static u16 CalculateGBABoxMonChecksum(GBABoxPokemon *gbaBoxMon)
{
    ...
    for (int i = 0; i < (int)NELEMS(substruct0->raw); i++) {
        checksum += substruct0->raw[i];
    }

    for (int i = 0; i < (int)NELEMS(substruct1->raw); i++) {
        checksum += substruct1->raw[i];
    }

    for (int i = 0; i < (int)NELEMS(substruct2->raw); i++) {
        checksum += substruct2->raw[i];
    }

    for (int i = 0; i < (int)NELEMS(substruct3->raw); i++) {
