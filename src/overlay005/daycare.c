...
int BoxPokemon_GiveExperience(BoxPokemon *boxMon, u32 givenExp)
{
    ...
    exp = BoxPokemon_GetData(boxMonRef, MON_DATA_EXP, NULL);
    exp += givenExp;

    BoxPokemon_SetData(boxMonRef, MON_DATA_EXP, (u8 *)&exp);
            ...
            for (j = 0; j < eggMoveCount; j++) {
                if (builder->fatherMoves[i] == builder->eggSpeciesEggMoves[j]) {
                    if (Pokemon_AddMove(egg, builder->fatherMoves[i]) == LEARNSET_ALL_SLOTS_FILLED) {
                        Pokemon_AppendMove(egg, builder->fatherMoves[i]);
                    }
                    break;
                }
            }
            ...
            if (builder->eggSpeciesLevelUpMoves[j] != MOVE_NONE) {
                if (builder->sharedMoves[i] == builder->eggSpeciesLevelUpMoves[j]) {
                    if (Pokemon_AddMove(egg, builder->sharedMoves[i]) == LEARNSET_ALL_SLOTS_FILLED) {
                        Pokemon_AppendMove(egg, builder->sharedMoves[i]);
                    }
                    break;
                }
            }
...
static void Egg_TryGiveVoltTackle(Pokemon *mon, Daycare *daycare)
{
    ...
    if (item1 == ITEM_LIGHT_BALL || item2 == ITEM_LIGHT_BALL) {
        if (Pokemon_AddMove(mon, MOVE_VOLT_TACKLE) == LEARNSET_ALL_SLOTS_FILLED) {
            Pokemon_AppendMove(mon, MOVE_VOLT_TACKLE);
        }
    }
}
...
void Egg_CreateEgg(Pokemon *egg, u16 species, u8 param2, TrainerInfo *trainerInfo, int param4, int metLocation)
{
    ...
    if (param2) {
        Pokemon_SetData(egg, MON_DATA_MET_LOCATION, &param2);
    }
    ...
    Pokemon_SetData(egg, MON_DATA_NICKNAME_STRBUF, eggName);
    Strbuf_Free(eggName);
    ...
    if (param4 == 4) {
        ...
        Pokemon_SetData(egg, MON_DATA_OTNAME_STRBUF, otName);
...
static void Egg_SetInitialData(Pokemon *mon, u16 species, Daycare *daycare, u32 monOTID, u8 form)
{
    ...
    Pokemon_SetData(mon, MON_DATA_NICKNAME_STRBUF, strBuf);
    Strbuf_Free(strBuf);
}
...
static int Party_GetEggCyclesToSubtract(Party *party)
{
    ...
    for (i = 0; i < partyCount; i++) {
        if (Pokemon_GetData(Party_GetPokemonBySlotIndex(party, i), MON_DATA_EGG_EXISTS, NULL) == FALSE) {
    ...
    if (steps == Daycare_GetEggCycleLength(fieldSystem)) {
        ...
        for (i = 0; i < Party_GetCurrentCount(party); i++) {
            ...
            if (Pokemon_GetData(mon, MON_DATA_IS_EGG, NULL)) {
                if (Pokemon_GetData(mon, MON_DATA_IS_DATA_INVALID, NULL)) {
                    continue;
                }
...
static void Egg_CreateHatchedMonInternal(Pokemon *egg, int heapID)
{
    ...
    for (i = 0; i < LEARNED_MOVES_MAX; i++) {
        ...
        movesPP[i] = Pokemon_GetData(egg, MON_DATA_MOVE1_CUR_PP + i, NULL);
    }
    ...
    if (species == SPECIES_MANAPHY) {
        if (Pokemon_GetData(egg, MON_DATA_MET_LOCATION, NULL) == SpecialMetLoc_GetId(2, 1)) {
    ...
    for (i = 0; i < LEARNED_MOVES_MAX; i++) {
        ...
        Pokemon_SetData(mon, MON_DATA_MOVE1_CUR_PP + i, &(movesPP[i]));
    }
    ...
    Pokemon_SetData(mon, MON_DATA_MARKS, &marks);
    ...
    Pokemon_SetData(mon, MON_DATA_OTNAME_STRBUF, strBuf);
    Pokemon_SetData(mon, MON_DATA_OT_GENDER, &gender);
    Pokemon_SetData(mon, MON_DATA_OT_ID, &otID);
    Pokemon_SetData(mon, MON_DATA_FORM, &form);

    u16 location = Pokemon_GetData(egg, MON_DATA_MET_LOCATION, NULL);
    u8 year = Pokemon_GetData(egg, MON_DATA_MET_YEAR, NULL);
    u8 month = Pokemon_GetData(egg, MON_DATA_MET_MONTH, NULL);
    u8 day = Pokemon_GetData(egg, MON_DATA_MET_DAY, NULL);
    ...
    location = Pokemon_GetData(egg, MON_DATA_HATCH_LOCATION, NULL);
    year = Pokemon_GetData(egg, MON_DATA_HATCH_YEAR, NULL);
    month = Pokemon_GetData(egg, MON_DATA_HATCH_MONTH, NULL);
    day = Pokemon_GetData(egg, MON_DATA_HATCH_DAY, NULL);

    Pokemon_SetData(mon, MON_DATA_HATCH_LOCATION, &location);
    Pokemon_SetData(mon, MON_DATA_HATCH_YEAR, &year);
    Pokemon_SetData(mon, MON_DATA_HATCH_MONTH, &month);
    Pokemon_SetData(mon, MON_DATA_HATCH_DAY, &day);
