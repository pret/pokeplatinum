...
static void AreaData_StripTextureData(void *ressourceFile, NNSG3dResTex *texture)
{
    ...
    u32 strippedTextureDataSize = (u32)(textureData - (u8 *)ressourceFile);

    Heap_ReallocFromHeap(ressourceFile, strippedTextureDataSize);
}
...
void AreaDataManager_Load(AreaDataManager *areaDataManager)
{
    ...
    // If no map prop model was loaded, load the model of a dummy box ("dmybox00")
    ...
    sprintf(mapPropMaterialShapeFilePath, MAP_PROP_MATERIAL_SHAPE_FILE_PATH);
