...
void ov5_021DDAA4 (FieldSystem * fieldSystem)
{
    ...
    if (MapHeader_GetMapLabelWindowID(fieldSystem->location->unk_00) == 0) {
        return;
    }

    if (!sub_0203A288(fieldSystem->location->unk_00)) {
        v0 = MapHeader_GetMapLabelTextID(fieldSystem->location->unk_00);
        v1 = MapHeader_GetMapLabelWindowID(fieldSystem->location->unk_00);
