...
{
    MapPropMaterialShape *propMatShp = Heap_AllocFromHeap(HEAP_ID_FIELD1, sizeof(MapPropMaterialShape));
...
void MapPropMaterialShape_Load(const char *path, MapPropMaterialShape *propMatShp)
{
    ...
    if (FS_OpenFile(&file, path)) {
        ...
        propMatShp->idsLocators = Heap_AllocFromHeap(HEAP_ID_FIELD1, idsLocatorsSize);
        GF_ASSERT(propMatShp->idsLocators != NULL);

        propMatShp->ids = Heap_AllocFromHeap(HEAP_ID_FIELD1, idsSize);
