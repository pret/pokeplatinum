...
static BOOL sub_020529C4(FieldTask *task)
{
    ...
    switch (v0->unk_00) {
    case 0:
        StartScreenFade(FADE_MAIN_ONLY, FADE_TYPE_BRIGHTNESS_IN, FADE_TYPE_MAX, FADE_TO_BLACK, 8, 1, HEAP_ID_FIELD_TASK);
        v0->unk_00++;
        break;
    case 1:
        if (IsScreenFadeDone()) {
    ...
    case 2:
        if ((gSystem.pressedKeys & PAD_BUTTON_A) || (gSystem.pressedKeys & PAD_BUTTON_B)) {
            StartScreenFade(FADE_BOTH_SCREENS, FADE_TYPE_BRIGHTNESS_OUT, FADE_TYPE_BRIGHTNESS_OUT, FADE_TO_BLACK, 8, 1, HEAP_ID_FIELD_TASK);
            v0->unk_00++;
        }
        break;
    case 3:
