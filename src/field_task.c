...
{
    FieldTask *task = Heap_AllocFromHeapAtEnd(HEAP_ID_FIELD3, sizeof(FieldTask));
    task->prev = NULL;
    task->func = taskFunc;
    task->state = 0;
    task->env = taskEnv;
    ...
    task->dummy1C = Heap_AllocFromHeapAtEnd(HEAP_ID_FIELD3, sizeof(u32));
...
{
    FieldTaskEnv *env = Heap_AllocFromHeapAtEnd(HEAP_ID_FIELD3, sizeof(FieldTaskEnv));
    env->state = 0;
