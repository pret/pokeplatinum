...
PokemonInfoDisplayStruct *sub_02092494(Pokemon *mon, BOOL isMine, int heapID)
{
    ...
    ptr->heapID = heapID;
    ptr->msgData = MessageLoader_Init(MSG_LOADER_LOAD_ON_DEMAND, NARC_INDEX_MSGDATA__PL_MSG, TEXT_BANK_POKEMON_SUMMARY_SCREEN, ptr->heapID);
...
static void FormatDateAndLocationMet(PokemonInfoDisplayStruct *infoDisplay, int msgNo)
{
    ...
    StringTemplate_SetNumber(infoDisplay->msgFmt, 0, Pokemon_GetValue(infoDisplay->mon, MON_DATA_MET_YEAR, NULL), 2, 2, 1);
    StringTemplate_SetMonthName(infoDisplay->msgFmt, 1, Pokemon_GetValue(infoDisplay->mon, MON_DATA_MET_MONTH, NULL));
    StringTemplate_SetNumber(infoDisplay->msgFmt, 2, Pokemon_GetValue(infoDisplay->mon, MON_DATA_MET_DAY, NULL), 2, 0, 1);
    StringTemplate_SetNumber(infoDisplay->msgFmt, 3, Pokemon_GetValue(infoDisplay->mon, MON_DATA_MET_LEVEL, NULL), 3, 0, 1);
    StringTemplate_SetMetLocationName(infoDisplay->msgFmt, 4, Pokemon_GetValue(infoDisplay->mon, MON_DATA_MET_LOCATION, NULL));
    StringTemplate_SetNumber(infoDisplay->msgFmt, 5, Pokemon_GetValue(infoDisplay->mon, MON_DATA_EGG_YEAR, NULL), 2, 2, 1);
    StringTemplate_SetMonthName(infoDisplay->msgFmt, 6, Pokemon_GetValue(infoDisplay->mon, MON_DATA_EGG_MONTH, NULL));
    StringTemplate_SetNumber(infoDisplay->msgFmt, 7, Pokemon_GetValue(infoDisplay->mon, MON_DATA_EGG_DAY, NULL), 2, 0, 1);
    StringTemplate_SetMetLocationName(infoDisplay->msgFmt, 8, Pokemon_GetValue(infoDisplay->mon, MON_DATA_EGG_LOCATION, NULL));
    ...
    if (Pokemon_GetValue(mon, MON_DATA_IS_EGG, NULL) == 0) {
        if (Pokemon_GetValue(mon, MON_DATA_EGG_LOCATION, NULL) == 0) {
            ...
            if (Pokemon_GetValue(mon, MON_DATA_FATEFUL_ENCOUNTER, NULL) == 1) {
                ...
                if ((Pokemon_GetValue(mon, MON_DATA_EGG_LOCATION, NULL) == SpecialMetLoc_GetId(1, 1))
                    || (Pokemon_GetValue(mon, MON_DATA_EGG_LOCATION, NULL) == SpecialMetLoc_GetId(1, 0))
                    || (Pokemon_GetValue(mon, MON_DATA_EGG_LOCATION, NULL) == SpecialMetLoc_GetId(1, 9))
                    || (Pokemon_GetValue(mon, MON_DATA_EGG_LOCATION, NULL) == SpecialMetLoc_GetId(1, 10))
                    || (Pokemon_GetValue(mon, MON_DATA_EGG_LOCATION, NULL) == SpecialMetLoc_GetId(1, 11))) {
                    if (param1 == 1) {
                        v0 = 5;
                    } else {
                        v0 = 6;
